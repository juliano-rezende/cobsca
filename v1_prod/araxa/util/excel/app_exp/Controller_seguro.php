...
/*
SELECT
  `faturamentos`.`matricula`, `associados`.`nm_associado`, `associados`.`cpf`,
  `FLOOR`(DateDiff(Now(), `associados`.`dt_nascimento`) / 365) AS `idade`, CASE
    WHEN `associados`.`estado_civil` = 'c' THEN 'CASADO(A)'
    WHEN `associados`.`estado_civil` = 's' THEN 'SOLTEIRO(A)'
    WHEN `associados`.`estado_civil` = 'v' THEN 'VIÚVO(A)'
    WHEN `associados`.`estado_civil` = 'e' THEN 'AMASIADO(A)'
    WHEN `associados`.`estado_civil` = 'd' THEN 'DIVORCIADO(A)'
    ELSE 'NÃO INFORMADO' END AS `estadoCivil`, `associados`.`dt_nascimento`
FROM
  `faturamentos` INNER JOIN
  `associados` ON `faturamentos`.`matricula` = `associados`.`matricula`
  INNER JOIN
  `dados_cobranca` ON `faturamentos`.`dados_cobranca_id` = `dados_cobranca`.`id`
  INNER JOIN
  `planos` ON `dados_cobranca`.`planos_id` = `planos`.`id`
WHERE
  `faturamentos`.`referencia` = '2020-01-01' AND
  `planos`.`seguro` = 1 AND
  `faturamentos`.`status` = 1;
*/
// bibliotecas
