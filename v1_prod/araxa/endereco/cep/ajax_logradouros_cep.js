...
var cep = jQuery("input[name=cep"+campo+"]").val().replace(/\D/g, '');
	...
	if (cep != "") {
		...
		if(validacep.test(cep)) {
			...
			if (("erro" in dados)) {
					//cep é inválido.
					UIkit.notify("<i class='uk-icon-info-circle' ></i> Cep não encontrado selecione diretamento o estado.", {status:'danger',timeout: 2500});
					jQuery("select[name=uf"+campo+"]").focus(); //'select#ex1 option:eq(5)'
					modal.hide();// fecha msg na tela
					return;
			}if (dados.logradouro =="" && dados.complemento =="" && dados.bairro =="") {
				//cep é inválido.
					UIkit.notify("<i class='uk-icon-info-circle' ></i> Cidade com cep padrão selecione diretamento o estado.", {status:'info',timeout: 2500});
