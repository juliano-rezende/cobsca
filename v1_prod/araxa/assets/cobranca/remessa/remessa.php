        ...
        <form name="FrmFiltroRemessa" method="post" id="FrmFiltroRemessa" class="uk-form">
            <fieldset style=" width:450px;border:0; padding:0; padding-top:0px;">
                <label id="lab00">  <!--pesquisar por conta bancaria -->
                    <span>Banco</span>
                    <select name="cod_banco_grid" class="select w_300" id="cod_banco_grid">
    ...
    <div class="uk-modal-dialog" style="width:500px;">
        ...
        <form name="FrmGeraRemessa" method="post" id="FrmGeraRemessa" class="uk-form">
            <fieldset style=" width:450px;border:0; padding:0; padding-top:0px;">
                <label id="lab00">  <!--pesquisar por conta bancaria -->
                    <span>Banco</span>
                    <select name="cod_banco_rem" class="select w_300" id="cod_banco_rem">
                        <option value="" selected></option>
                        <?php
                            ...
                            $listcontas = new ArrayIterator($contas);
                            while ($listcontas -> valid()):
                                echo '<option value="' . $listcontas -> current() -> cod_banco . '_' . $listcontas -> current() -> tipo_arquivo . '" uk-data-conta_id="' . tool ::Completazeros(3, $listcontas -> current() -> id) . '" >' . $listcontas -> current() -> id . " " . $listcontas -> current() -> cod_banco . " " . utf8_encode($listcontas -> current() -> nm_conta) . '</option>';
                                $listcontas -> next();
                            endwhile;
                        ?>
                    </select>
                ...
                <label>
                    <span>Dta Inicial</span>
                    <input type="text" class="input_text w_120 uk-text-center" name="dtinirem" id="dtinirem" data-uk-datepicker="{format:'DD/MM/YYYY'}" placeholder="00/00/0000"/>
                </label>
                <label>
                    <span>Dta Final</span>
                    <input type="text" class="input_text w_120 uk-text-center" name="dtinifim" id="dtinifim" data-uk-datepicker="{format:'DD/MM/YYYY'}" placeholder="00/00/0000"/>
                </label>
            </fieldset>
        </form>
        <div class="uk-modal-footer uk-text-right">
...
<div id="Grid_remessa" style="background-color: #fff; height:<?php echo tool ::HeightContent($COB_Heigth, $COB_Browser) - 80; ?>px;">
</div>
    ...
    jQuery(function () {
        ...
        jQuery("#BtnRem00").click(function (event) {
            ...
            jQuery.ajax({
                ...
                type: "post",
                data: jQuery("#FrmFiltroRemessa").serialize(),
        ...
        jQuery.ajax({
            ...
            success: function (res) {
                ...
                if (res == 1) {
                    ...
                    UIkit.notify('Não foi possível abrir o arquivo', {timeout: 2000, status: 'danger'});
                    modal.hide();

                } else if (res == 2) {

                    UIkit.notify('Não foi remover o arquivo texto', {timeout: 2000, status: 'danger'});
                    modal.hide();

                } else if (res == 3) {

                    UIkit.notify('' + obj.msg + '', {timeout: 2000, status: '' + obj.status + ''});
                    $.post("assets/cobranca/remessa/ajax_grid_remessa.php", {cod_banco_grid: obj.cod_banco, pagina: 1},
                        function (data) {
                    ...
                    UIkit.notify('Não foi possível criar o arquivo zip', {timeout: 2000, status: 'danger'});
                    modal.hide();

                } else if (res == 5) {

                    UIkit.notify('Não a registros para remessa!', {timeout: 2000, status: 'danger'});
                    modal.hide();
