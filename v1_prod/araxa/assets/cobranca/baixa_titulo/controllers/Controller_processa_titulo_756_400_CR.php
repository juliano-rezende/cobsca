...
if($FRM_action == "pay"){
	...
	$vlr_valor_pago     = tool::limpaMoney($FRM_vlr_total) - tool::limpaMoney($FRM_vlr_descontos);
	$vlr_acrescimos     = tool::limpaMoney($FRM_vlr_total) - tool::limpaMoney($FRM_vlr_nominal);
	...
	if($FRM_tpbaixa == 1){
		...
		$detalhes_C='CREDITO RECEBIMENTO BANCARIO  NÃO INFORMADO NO RETORNO';

		// cria o historico do lançamento no debito
		$historico_D='DEBITO TARIFA BANCARIA.';
		$detalhes_D='DEBITO TARIFA COMPENSAÇÃO BANCARIA NÃO INFORMADA NO RETORNO';
		...
		$caixa = caixa::create(
	 	...
	 	// recupera o centro de custo e plano de conta padrao da conta
	  	...
	  	if(!$Query_conta_bancaria){
	  		echo "<div class='uk-text-muted uk-text-success'>Erro interno. Valor lançado em conta padrão Dinheiro .</br></div>";
	  		$conta_bank=1;
	  	}else{

	  		$conta_bank=$Query_conta_bancaria->formas_recebimentos_id;
	  	}
	  	...
	  	$caixa = caixa::create(
	                        array('historico'                 => $historico_C,
	                              ...
	                              'tipolancamento'            => 1,
	                              'tipo'                      => 'c',
	                              'formas_recebimentos_id'     => $conta_bank,
	                              'clientes_fornecedores_id'  => "2.0000000000", // SERÁ ADIONADO O ZERO ANTES NA PRIMEIRA PÓSIÇÃO INDICANDO QUE É RECIMENTO DE BOLETO BANCARIOS
	                              'centros_custos_id'         => $dados_config->centros_custos_id,
	                              'planos_contas_id'          => $dados_config->planos_contas_id,
	...
	while($list_fat->valid()){
	    ...
	    $up_faturamento->update_attributes(
	  	...
	  	if($procedimentos){
		    ...
		    if(!$up_pro){echo "<div class='uk-text-muted uk-text-success'> Erro ao realizar Cancelamento do procedimento!</br></div>";}
