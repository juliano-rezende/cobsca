...
(SELECT  count(id) FROM faturamentos WHERE dt_pagamento between '".($A)."-01-01' AND '".(date("Y")-1)."-01-31' AND status='1' AND empresas_id='".$COB_Empresa_Id."') as rjan16,
...
$next_month=date('Y-m-d', strtotime('+2 months', strtotime(date('Y-m')."-01")));
...
$query_seguro=seguros::find_by_sql("SELECT SQL_CACHE
										...
										WHERE
										seguros.referencia =  '".date("Y-m")."-01'
										ORDER BY
...
<div id="big_col_titulos" class="uk-width-1-1 uk-scrollspy-inview uk-animation-slide-left" style="height:390px; background-color: #f5f5f5;" >

    <div class="uk-grid uk-width"  style="padding:10px; width: 90%; margin: 0 auto; ">
        <div class="uk-width-medium-1-9 uk-row-first">
            <div class="uk-panel uk-panel-box">
                <h3 class="uk-panel-title uk-text-primary">
                	<i class="uk-icon-barcode" ></i> Titulos Emitidos em 
                	<?php echo tool:: MesExtenso(date("m"))."/".date("Y"); ?>
                	<!--<i class="uk-icon-print uk-icon-small" onclick="Print('big_col_titulos');" style="cursor: pointer; float: right;" data-uk-tooltip="" title="" data-cached-title="Imprimir listagem"></i>-->
                </h3>
					...
					<table  class="uk-table" style="border-top: 1px solid #ccc;" >
					   <thead>
					      <tr style="line-height:25px;">
					        ...
					        <th class="uk-width uk-text-center" style="width:100px;">Numero doc</th>
					        <th class="uk-width uk-text-center" style="width:100px;">Nosso Numero</th>
					        <th class="uk-text-left"  >Sacado</th>
					        <th class="uk-width uk-text-center" style="width:150px;" >Dt Emissão</th>
					        <th class="uk-width uk-text-center" style="width:100px;"  >Dt Vencimento</th>
					        <th class="uk-width uk-text-center" style="width:100px;"  >valor</th>
					        <th class="uk-width" style="width:100px;"></th>
					      </tr>
					    </thead>
					</table>
...
<?php
$t=1;
while($list_titulos->valid()):


// data da ultima parcela paga
$dt_emis = new ActiveRecord\DateTime($list_titulos->current()->dt_emissao);
$dt_venc = new ActiveRecord\DateTime($list_titulos->current()->dt_vencimento);
$dt_ref = new ActiveRecord\DateTime($list_titulos->current()->referencia);
...
?>
							<tr style="line-height:25px;" >
								...
								<td class="uk-width uk-text-center" style="width:100px;" >
									<?php echo tool::CompletaZeros(10,$list_titulos->current()->numero_doc); ?>
								</td>
								<td class="uk-width uk-text-center" style="width:100px;" >
									<?php echo tool::CompletaZeros(10,$list_titulos->current()->nosso_numero)."-".$list_titulos->current()->dv_nosso_numero; ?>
								</td>
								<td class="uk-text-left uk-text-uppercase "  >
									<?php echo strtoupper($list_titulos->current()->sacado); ?>
									- (<?php if($list_titulos->current()->cod_remessa > 0){echo 'Transmitido banco!';} ?>)
								</td>
								<td class="uk-width uk-text-center" style="width:150px;" ><?php echo $dt_emis->format('d/m/Y');?></td>
								<td class="uk-width uk-text-center" style="width:100px;" ><?php echo $dt_venc->format('d/m/Y');?></td>
								<td class="uk-width uk-text-center" style="width:100px;" ><?php echo number_format($list_titulos->current()->vlr_nominal,2,',','.'); ?></td>
								<td class="uk-width " style="text-align: right;width:90px;" onclick="viewTitulos('<?php echo $list_titulos->current()->cod_banco; ?>','<?php echo $list_titulos->current()->convenios_id; ?>','<?php echo $dt_ref->format('Ymd');?>','<?php echo $list_titulos->current()->id; ?>');">
									<a   data-uk-tooltip="" title="Reimpressão de titulo"><i class="uk-icon-print uk-icon-small "></i></a>
								</td>
							</tr>

<?php $t++; $list_titulos->next(); endwhile;	?>
						</tbody>
					</table>
					</div>
                </div>
        ...
        <div class="uk-width-medium-1-9 uk-row-first">
            <div class="uk-panel uk-panel-box">
                <h3 class="uk-panel-title uk-text-primary">
                	...
                	<i class="uk-icon-print uk-icon-small" onclick="Print('big_col_2_grid');" style="cursor: pointer; float: right;" data-uk-tooltip="" title="" data-cached-title="Imprimir listagem"></i>
                </h3>
                <div class="uk-panel uk-text-left" style="height: 300px; overflow-y:none; width:100%; padding-right: 3px; ">
					<table  class="uk-table" style="border-top: 1px solid #ccc;" >
					   <thead>
        ...
        <div class="uk-width-medium-1-9 uk-row-first" >
            <div class="uk-panel uk-panel-box">
                <h3 class="uk-panel-title uk-text-primary">
                	...
                	<i class="uk-icon-print uk-icon-small " onclick="Print('big_col_6_grid');" style="cursor: pointer; float: right; margin-left: 4px;" data-uk-tooltip="" title="" data-cached-title="Imprimir "></i>
