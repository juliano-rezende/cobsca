...
$Query_all=faturamentos::find_by_sql("SELECT titulos_bancarios.vlr_tarifa,associados.nm_associado,faturamentos.matricula,faturamentos.dt_vencimento,faturamentos.dt_pagamento,faturamentos.valor,faturamentos.obs,convenios.nm_fantasia
                                      FROM faturamentos
                                      RIGHT JOIN associados ON faturamentos.matricula = associados.matricula
                                      RIGHT JOIN titulos_bancarios ON titulos_bancarios.id=faturamentos.titulos_bancarios_id
                                      RIGHT JOIN convenios ON associados.convenios_id = convenios.id
                                      WHERE faturamentos.dt_pagamento between '".$FRM_referencia."-01' AND '".$FRM_referencia."-31' AND faturamentos.status='1' AND faturamentos.empresas_id='".$COB_Empresa_Id."'");
...
<tfoot>
  <tr style="line-height:25px;">
    ...
    <th class="uk-text-center"  >Média tarifas <?php echo number_format($tarifa/$linha,2,',','.'); ?></th>
  </tr>
</tfoot>
</table>
</div>
