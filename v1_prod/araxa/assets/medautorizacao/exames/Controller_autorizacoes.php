...
if($FRM_action == 0){
	...
	$FRM_med_areas_id			=	isset( $_POST['areaId'])	? $_POST['areaId']: tool::msg_erros("O Campo Obrigatorio codigo da area Faltando.");
	...
	$QueryAutoricacao	= med_autorizacoes::create(
		array(
			...
			'hr_realizacao' 		=>$FRM_hr_realizacao,
			'med_parceiros_id' 		=>$FRM_med_parceiros_id,
			'med_areas_id' 			=>$FRM_med_areas_id,
	...
	if($QueryAutoricacao==true){
		$last=med_autorizacoes::find("last");//recupera o ultimo id
		echo intval($last->id);
	}else{
