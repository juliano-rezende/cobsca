...
<div id="BigGridAut" style="height:500px; margin: 0; padding: 10px;">
    ...
    <form class="uk-form uk-form-tab" id="Frmautexames" style="padding:0; margin-top: 0; ">
      <label>
        <span>Codigo</span>
...
<div style="height: 200px; width: 99.9%; border: 1px solid #ccc; margin-bottom: 15px;" id="GridProAut"><span style="margin: 0 45%;">Carregando...</span></div>
...
jQuery("#GridProAut").load('assets/medautorizacao/exames/Grid_procedimentos.php?autId='+jQuery("#autId").val()+'');
...
/********************************************************************************************************/
  jQuery("#parId").change(function(){
    ...
    jQuery.post("assets/medautorizacao/ajax/ajax_especialidades.php",
      {parId:jQuery(this).val(),areaId:2},
...
New_window('list','950','550','Impressão','assets/medautorizacao/print_aut_exames.php?autId=<?php  if(isset($dadosautorizacao)){ echo tool::CompletaZeros("11",$dadosautorizacao->id); }; ?>',true,false,'Carregando...');
});
...
jQuery(function() {
  ...
  event.preventDefault();
    ...
    url: "assets/medautorizacao/Controller_autorizacoes.php",
    type: "post",
    data:"action=5&mat=<?php echo $FRM_matricula; ?>&"+jQuery("#Frmautexames").serialize(),
    success: function(resultado) {
        if(jQuery.isNumeric(resultado)){
            // mensagen de carregamento
            jQuery("#msg_loading").html(" Carregando ");
            ...
            New_window('list','700','550','Solicitar Autorização','assets/medautorizacao/Frm_aut_exames.php?matricula=<?php echo $FRM_matricula; ?>&autId='+autId+'',true,false,'Carregando...');
...
function autocompletPro() {
  ...
  if(keywordpro!=""){
    ...
    if (keywordpro.length >= min_length) {
      ...
      jQuery.ajax({
        url: 'assets/medautorizacao/exames/ajax_refresh_procedimento.php',
        type: 'POST',
...
jQuery.post("assets/medautorizacao/exames/Controller_procedimentos.php",
...
function(resultado){
  ...
  if(jQuery.isNumeric(resultado)){
      ...
      jQuery("#GridProAut").load('assets/medautorizacao/exames/Grid_procedimentos.php?autId='+autId+'',function(){
