...
$queryfatu=faturamentos::find_by_sql("SELECT SQL_CACHE
									...
									WHERE
									  faturamentos.matricula = '".$FRM_matricula."' AND  faturamentos.status != '3'
									ORDER BY
									  faturamentos.referencia");
...
<table  class="uk-table" >
  <thead >
    <tr style="line-height: 30px;">
		...
		<th class="uk-width uk-text-center" style="width:100px;"  data-uk-tooltip="" title="Codigo da parcela" data-cached-title="Codigo da parcela">Parcela Nº</th>
		...
		<th class="uk-width uk-text-center" style="width:107px;" data-uk-tooltip="" title="Banco Boleto" data-cached-title="Banco Boleto" >Ações</th>
  ...
  ?>
    <tr class="all <?php echo $tag; ?>" data-id="<?php echo $listfat->current()->id; ?>" style="line-height: 30px"  >
       ...
       /* echo tool::CompletaZeros(10,$listfat->current()->nosso_numero)."-".$listfat->current()->dv_nosso_numero;*/
		...
		<td lass="uk-width uk-text-center" style="width:100px;" >
       		<?php
...
var autorizacao = <?php echo $COB_Faturamento; ?>;


if(autorizacao == 0){UIkit.notify("Você não possui permissões suficientes!", {status:'danger',timeout: 2500}); return false;}
...
var id_parcela = jQuery(this).closest('tr').attr('data-id');//pega a linha de exibição
	...
	New_window('file-text-o','520','460','Detalhes da parcela','assets/faturamento/Frm_detalhes.php?paid='+id_parcela+'',true,false,'Carregando parcela...');
});
