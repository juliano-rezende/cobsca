...
$Query_fpgto=formas_pagamentos::find_by_sql("SELECT
                                              formas_pagamentos.id AS fpgto_emp_id,
                                              formas_pagamento_sys.descricao,
                                              formas_pagamento_sys.id as f_pgto_sys_id
                                            FROM
                                              formas_pagamentos
                                              LEFT JOIN formas_pagamento_sys ON formas_pagamento_sys.id =
                                            formas_pagamentos.formas_pagamento_sys_id
                                            WHERE
                                              formas_pagamentos.status = 1;");
$formas= new ArrayIterator($Query_fpgto);
...
<fieldset style="width:300px; background-color:transparent;">
    ...
    <span>Forma Receb</span>
        ...
        <?php
            while($formas->valid()):
            echo'<option value="'.$formas->current()->fpgto_emp_id.'" >'.utf8_encode(strtoupper($formas->current()->descricao)).'</option>';
            $formas->next();
            endwhile;
        ?>
        </select>
