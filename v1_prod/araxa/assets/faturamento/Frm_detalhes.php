...
<ul class="uk-tab uk-gradient-cinza" data-uk-tab >
    <li class="uk-active"><a uk-data-tab="#Geral" >Dados Parcela</a></li>
    <li><a uk-data-tab="#Detalhes">Dados Boleto</a></li>
    <li><a uk-data-tab="#Procedimentos" >Gastos Clinícos</a></li>
</ul>
...
<div style="height:30px; width:260px; position:absolute; top:78px; right:20px; text-align:center;">
    <label>
...
<div style="height:30px; width:260px; position:absolute; top:114px; right:20px; text-align:center;">
 <label>
<span>Nº Boleto</span>
...
<div style="height:30px; width:260px; position:absolute; top:151px; right:20px; text-align:center;">
<label>
	<span>DT Pagamento</span>
	<input type="text" class="uk-text-right w_120" readonly="readonly"  value="<?php if($Query_fat[0]->dt_pagamento==""){echo"00/00/0000";}else{echo $dtpgto->format('d/m/Y');} ?>" />
</label>
</div>
...
<label>
<span>Valor</span>
<input type="text" style=" background-color: #fff; color: #000;" <?php if($Query_fat[0]->titulos_bancarios_id > 0 ){echo 'disabled="disabled"';}?>" class="uk-text-right w_120 uk-badge-primary" value="<?php echo number_format($Query_fat[0]->valor,2,",","."); ?>" id="vlr" />
</label>
...
<div style="height:30px; width:260px; position:absolute; top:187px; right:20px; text-align:center;">
<label>
<span>Descontos</span>
...
<label>
 ...
 <label>
    ...
    <?php if($Query_fat[0]->cod_retorno == 0 && $Query_fat[0]->nosso_numero == 0){
        ...
        echo"Parcela transmitida para o banco!";

        }
        if($Query_fat[0]->cod_retorno > 0 && $Query_fat[0]->nosso_numero > 0){ echo $Query_fat[0]->obs_titulo;}  ?>

    </div>
</label>
<label>
<span>Observações</span>
<textarea  <?php if($Query_fat[0]->titulos_bancarios_id > 0 ){echo 'disabled="disabled"';}?> id="dth" style="height: 120px; width: 380px; background-color: #fff; color: #000;" ><?php echo  str_replace("<br>", "\n", $Query_fat[0]->obs);?></textarea>
</label>
...
if($Query_fat[0]->cod_remessa <= "0"){
    ...
    if($COB_Faturamento == 1){
?>

<a  id="Btn_det_0" class="uk-button uk-button-primary" style=" bottom:40px; position:absolute;  right:15px;" data-uk-tooltip="{pos:'left'}" title="Atualizar" data-cached-title="Atualizar" ><i class="uk-icon-check"></i> Atualizar </a>
...
if($listpro->current()->status == 0 ){$st= "A faturar";$class="uk-badge-warning";}
if($listpro->current()->status == 1 ){$st= "Faturado";$class="uk-badge";}
if($listpro->current()->status == 2 ){$st= "Cancelado";$class="uk-badge-danger";}
if($listpro->current()->status == 3 ){$st= "Pago";$class="uk-badge-success";}
...
<article class="uk-comment" id="line_proc_<?php echo $listpro->current()->id; ?>">
    <header class="uk-comment-header">
        ...
        <button class="uk-button uk-button-small uk-icon-remove uk-remove-proc uk-button-danger" type="button" data-uk-tooltip title="Remover"  data-uk-proc="<?php echo $listpro->current()->id; ?>" style="float: right;"> Remover</button>
    </header>
...
<label>
 ...
 <label>
	...
	<input  type="text" class="uk-text-center  w_120" readonly="readonly"  value="<?php if($Query_fat[0]->tipo_baixa == "B"){echo "Bancaria";}if($Query_fat[0]->tipo_baixa == "M"){echo "Manual";}else{echo "-";}?>" />
</label>
<div style="height:30px; width:260px; position:absolute; top:223px; right:20px; text-align:center;">
 <label>
	<span>Negociada ?</span>
	<input  type="text" class="uk-text-center  w_120" readonly="readonly"  value="<?php if($Query_fat[0]->negociada == "S"){echo "Sim";}else{echo "Não";}?>" />
</label>
</div>
...
<div style="height:30px; width:260px; position:absolute; top:258px; right:20px; text-align:center;">
 ...
 <label>
	...
	<input  type="text" class="uk-text-center  w_120" readonly="readonly"  value="<?php if($Query_fat[0]->tipo_parcela == "M"){echo "Mensalidade";}else{echo "Adesão";}?>" />
</label>
</div>
 <label>
	<span>Assegurada ?</span>
	<input  type="text" class="uk-text-left  w_350 <?php if($Query_fat[0]->status == "0"){echo "uk-text-danger uk-text-bold";}
        												elseif($Query_fat[0]->status == "1"){echo "uk-text-primary uk-text-bold";}
        												elseif($Query_fat[0]->status == "2"){echo "uk-text-sucsess uk-text-bold";}
        												elseif($Query_fat[0]->status == "3"){echo "uk-text-danger uk-text-bold";}
        												?>" readonly="readonly"  value="<?php   if 	  ($Query_fat[0]->status == "0"){echo "( NÃO ) ";}
        																						elseif($Query_fat[0]->status == "1"){echo "( SIM ) ";}
        																						elseif($Query_fat[0]->status == "2"){echo "( SIM ) ";}
        																						elseif($Query_fat[0]->status == "3"){echo "( NÃO ) ";}
