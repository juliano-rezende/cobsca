...
#################################################################################################################################################################
/*validação da data de vencimento com a data de emissão*/

// Comparando as Datas
if( strtotime( date("Y-m-d") ) >= strtotime("".$Query_faturamento->dt_vencimento."") ){

$query_success .=("<div class='uk-text-danger'><i class='uk-icon-exclamation-triangle '></i> Erro ao gerar titulo para parcela nº ".$value.". data de vencimento menor que a data de emissão!</div></br>");
continue;
	...
	if($Query_faturamento->titulos_bancarios_id == 0 or $Query_faturamento->titulos_bancarios_id == ""){
		...
		$ult_id_titulos = titulos::last();
			...
			if(!$create_titulo){
				...
				$query_success .=("<div class='uk-text-success'><i class='uk-icon-check '></i> Titulo gerado com sucesso  para parcela nº ".$value.". !</div></br>");
				 }
	}else{
...
if($query_errors !=""){
	echo '":"","callback":"1","msg":"'.$query_errors.'","dir_bol":"'.$dadosconta[0]->dir_bol.'","titulos_id":"'.json_decode($titulos_id).'","status":"warning';
}else{
	echo 'callback":"0","msg":"'.$query_success.'","dir_bol":"'.$dadosconta[0]->dir_bol.'","titulos_id":"'.json_encode($titulos_id).'","status":"success';
}
