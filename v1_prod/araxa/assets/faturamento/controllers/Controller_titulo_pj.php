...
$FRM_subconvenio_id = isset( $_POST['subconvenio'])     ? intval($_POST['subconvenio'])                       /* variavel com o subconvenio das parcelas*/
: tool::msg_erros("O Campo subconvenio Obrigatorio faltando.");
...
if($FRM_subconvenio_id > 0){

  $query = "SELECT
  sub_convenios.razao_social,
  sub_convenios.cnpj,
  logradouros.descricao AS nm_logradouro,
  CASE
  WHEN sub_convenios.compl_end = '' THEN ' '
  ELSE sub_convenios.compl_end
  END AS compl_end ,
  sub_convenios.num,
  ...
  FROM
  sub_convenios
  LEFT JOIN logradouros ON logradouros.id = sub_convenios.logradouros_id
  ...
  WHERE
  sub_convenios.id = '".$FRM_subconvenio_id."'";
...
$sacado = convenios::find_by_sql($query);
