    ...
    <div id="menu-float" style="text-align:center;margin:0 900px;">
       ...
       <?php if(isset($dadosvendedor)):?>
        ...
        <a  id="Btn_vendor_2" class="uk-icon-button  uk-icon-pencil " style="margin-top:2px; text-align:center;" data-uk-tooltip="{pos:'left'}" title="Salvar Alterações" data-cached-title="Salvar Alterações" >
            <a  id="Btn_vendor_3" class="uk-icon-button uk-icon-list-ol" style="margin-top:2px;" data-uk-tooltip="{pos:'left'}" title="Convenios Vendedor" data-cached-title="Convenios vendedor" ></a>
...
<fieldset style="width:870px;">
  ...
  <label >
    ...
    <div class="uk-form-controls">
        <label >
            <input type="radio"  name="st" value="1" <?php  if(isset($dadosvendedor)){if($dadosvendedor[0]->status == "1"){echo"checked";} }?> > Ativo
            <input type="radio"  name="st" value="0" <?php  if(isset($dadosvendedor)){if($dadosvendedor[0]->status == "0"){echo"checked";} }?> > Inativo
        </label>
    </div>
</label>
<label>
...
<label>
    ...
    <select class="select w_400" name="cidade"  id="cidade" >
        <?php
        if(isset($dadosvendedor)):
            echo'<option value="'.$dadosvendedor[0]->cidade_id.'" selected="selected">'.utf8_encode(strtoupper($dadosvendedor[0]->nm_cidade)).'</option>';
        else:
...
<label>
    ...
    <select class="select w_400" name="bairro"  id="bairro" >
        <?php
        if(isset($dadosvendedor)):
            echo'<option value="'.$dadosvendedor[0]->bairro_id.'" selected="selected">'.utf8_encode(strtoupper($dadosvendedor[0]->nm_bairro)).'</option>';
        else:
            echo'<option value="" >Aguardando...</option>';
        endif;
        ?>
...
<label>
    ...
    <select class="select w_400" name="logradouro"  id="logradouro" >
        <?php
        if(isset($dadosvendedor)):
            echo'<option value="'.$dadosvendedor[0]->logradouros_id.'" selected="selected">'.utf8_encode(strtoupper($dadosvendedor[0]->nm_logradouro)).'</option>';
        else:
            echo'<option value="" >Aguardando...</option>';
        endif;
        ?>
