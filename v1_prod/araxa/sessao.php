...
if(isset($_SESSION[''.$Prefixo_SYS.'logado'])){
	...
	if(($created+$duraction) > time()){
			...
			$COB_Convenio_Id		=	$_SESSION[''.$Prefixo_SYS.'convenio_id'];			// id do convenio			
			$COB_Notificar			=	$_SESSION[''.$Prefixo_SYS.'notificar'] ;			//notificações usuario										
			$COB_Faturamento		=	$_SESSION[''.$Prefixo_SYS.'faturamento'];			//controles bancarios usuario
