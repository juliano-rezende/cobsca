...
if(isset($_POST['acao'])){
	...
	if(empty ($_POST['vl'])){
	    $query="SELECT SQL_CACHE  * FROM clientes_fornecedores WHERE  tipo='2' ORDER BY id asc ";
	// retorna a pesquisa pelo id enviado
	}elseif(is_numeric ($_POST['vl'])){
	    $query="SELECT SQL_CACHE  * FROM clientes_fornecedores WHERE id ='".tool::limpaString($_POST['vl'])."' and tipo='1'"; // pesquisa pelo id do cliente
	}elseif (ctype_alpha(tool::limpaString($_POST['vl']))) {
	    $query="SELECT SQL_CACHE  * FROM clientes_fornecedores WHERE (nm_cliente LIKE '".tool::limpaString($_POST['vl'])."%') or (nm_fantasia LIKE '".tool::limpaString($_POST['vl'])."%') and tipo='1' ORDER BY id asc"; // pesquisa pelo nome do cliente
		...
		if ( !$cpf_cnpj->valida() ) {
		   ...
		   $query="SELECT SQL_CACHE * FROM clientes_fornecedores WHERE cpf='".tool::limpaString($_POST['vl'])."' and tipo='1"; // pesquisa pelo cpf do cliente
		}
	    ...
	    $query="SELECT SQL_CACHE  * FROM clientes_fornecedores WHERE tipo='1' ORDER BY id asc  ";
}
// corrigi bug do active record
echo'<div class="tabs-spacer" style="display:none;">';
    ...
    <article class="uk-comment">
        <header class="uk-comment-header " style="padding: 5px;">
            ...
            <h4 class="uk-comment-title uk-text-bold">
				 <?php
					 if($clientes->current()->tp_pessoa == 1){echo $clientes->current()->nm_fantasia;}else{echo $clientes->current()->nm_cliente;}
				 ?>
            </h4>
