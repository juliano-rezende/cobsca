...
$FRM_data_cadastro		=	date("Y-m-d");
	...
	$query_duplicidade_assoc=associados::find_by_sql("SELECT matricula FROM associados WHERE cpf='".$FRM_cpf."'");
	...
	if(count($query_duplicidade_assoc) > 0){
		...
		$query_duplicidade_dep = dependentes ::find_by_sql("SELECT id FROM dependentes WHERE cpf='" . $FRM_cpf . "'");
...
//VALIDA O ENDEREÇO
if($FRM_logradouro==""  or $FRM_logradouro=="0" ){echo tool::msg_erros("é necessario selecionar um logradouro.");}
