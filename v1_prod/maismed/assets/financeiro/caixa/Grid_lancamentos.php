...
elseif($intervalo==7){
	$periodo="AND caixa.data Between '".date("Y-m-d")."' And '".date("Y-m-d")."'";
...
$user ="";
if($$COB_Usuario_Id == 1){
	$user = "AND usuarios_id='".$$COB_Usuario_Id."'";
}
...
$query= caixa::find_by_sql("SELECT SQL_CACHE
							...
							WHERE
							 ".$contabancaria." ".$tipo." ".$tipolancamento." ".$periodo."  AND caixa.empresas_id='".$COB_Empresa_Id."' '".$user." ' ORDER BY data,id ASC");
...
@media print {
	...
	#title_print > thead > tr > th {
		text-align:left;
		font-size: 0.7em;
		font-weight: bold;
	}

    #grid_fluxo_caixa{	font-size: 0.6em;}

    #grid_fluxo_caixa > tbody > tr > td{
        border-bottom:solid 1px #f5f5f5;
    }  

	.tdcenter{text-align:center;}

	#gridlancamentos{border:0;}
...
<table id="title_print">

  <thead>
    <tr style="line-height:30px;">
        <th class="uk-width" style="width:20px;text-align:center;"></th>
        <th class="uk-width tdcenter" style="width:85px;text-align:center;">Cod</th>
        <th class="uk-width tdcenter" style="width:85px;text-align:center;">Data</th>
        <th class="uk-width" style="width:265px;text-align:left;">Histórico</th>
        <th class="uk-width" style="width:220px;text-align:left;">Favorecido / Pagador </th>
        <th ></th>
        <th class="uk-width" style="width:120px;text-align:center;">F-Pgto</th>
        <th class="uk-width" style="width:120px;text-align:center;">Num Doc</th>
        <th class="uk-width" style="width:85px;text-align:center;">Entradas</th>
        <th class="uk-width" style="width:80px;text-align:center;">Saidas</th>
        <th class="uk-width" style="width:85px;text-align:center;">Saldo</th>
    </tr>
  </thead>
</table>

<table class="uk-table uk-table-striped uk-table-hover" id="grid_fluxo_caixa">
<tbody >
    <tr>
        <td class="uk-width uk-text-center" style="width:20px;" ></td>
		<td class="uk-width uk-text-center" style="width:90px;" ></td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" >
		  <?php
        ...
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" ></td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" ></td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" ></td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" ></td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" ><?php echo number_format($saldo,2,",","."); ?></td>
    </tr>
<?php
...
if($linhacaixa->current()->tipolancamento=='0'){
			...
			if($linhacaixa->current()->tipo=='d'){
				echo '<span style="color:#F00; font-size:12px; margin-top:5px;" title="Debito Transferencia" >-</span>';
				}else{
					echo '<spanstyle="color:#0CC; font-size:12px;margin-top:5px;" title="Credito Transferencia">+</span>';
					}
			}else{

			if($linhacaixa->current()->tipo=='d'){
				echo '<span style="color:#F00; font-size:12px;margin-top:5px;" title="Pagamento">-</span>';
			}else{
				echo '<span style="color:#0CC; font-size:12px;margin-top:5px;" title="Deposito">+</span>';
				}
}
		?>
        </th>
		<td class="uk-width uk-text-center tdcenter" style="width:90px;" ><?php echo tool::CompletaZeros(11,$linhacaixa->current()->id); ?></td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px;" >
        ...
        <td class="uk-text-center" style="color:#fff;" ></td>
        <td class="uk-width uk-text-center tdcenter" style="width:150px; max-width:150px;white-space: nowrap;text-overflow: ellipsis; overflow:hidden;" >
            <?php
            $formas=formas_recebimentos::find_by_sql("SELECT
                                                            formas_recebimento_sys.descricao
                                                         FROM formas_recebimentos
                                                         INNER JOIN formas_recebimento_sys ON formas_recebimento_sys.id = formas_recebimentos.formas_recebimento_sys_id
                                                         WHERE
                                                          formas_recebimentos.id = '".$linhacaixa->current()->formas_recebimentos_id."' ");
            echo $formas[0]->descricao;
            ?>
        </td>
        <td class="uk-width uk-text-center tdcenter" style="width:150px; max-width:150px;white-space: nowrap;text-overflow: ellipsis; overflow:hidden;" >
           <?php echo strtoupper($linhacaixa->current()->numdoc); ?>
        </td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px; color:#06F;" >
          <?php if($linhacaixa->current()->tipo=='c'){echo number_format($linhacaixa->current()->valor,2,",",".");}else{echo '-';} ?>
        </td>
        <td class="uk-width uk-text-center tdcenter" style="width:100px; color:#F00;" >
          <?php if($linhacaixa->current()->tipo=='d'){echo number_format($linhacaixa->current()->valor,2,",",".");}else{echo '-';} ?>
        </td>
        <td class="uk-width uk-text-center <?php echo $style_sd_dia; ?>  tdcenter" style="width:100px;" >
