...
$formas_pagamentos_id 		= isset( $_POST['formas_pagamentos_id'])	? $_POST['formas_pagamentos_id']:  		$erro="O Campo Obrigatorio 0-5 Faltando.";
...
$vlr_nominal=$vlr_nominal/$qte_p;	//valor da parcela
	...
	for($parcela=1; $parcela<=$qte_p; $parcela++){
		...
		$create= contas_pagar::create(array(
...
$cpagar_id 					= isset( $_POST['cpagar_id'])				? $_POST['cpagar_id']: 					$erro="O Campo Obrigatorio 1-0 Faltando.";
...
$formas_pagamentos_id 		= isset( $_POST['formas_pagamentos_id'])? $_POST['formas_pagamentos_id']:  		$erro="O Campo Obrigatorio 1-5 Faltando.";
...
$update=contas_pagar::find($cpagar_id);
...
$cpagar_id 					= isset( $_POST['cpagar_id'])				? $_POST['cpagar_id']: 					$erro="O Campo Obrigatorio 2-0 Faltando.";

$remover = contas_pagar::find($cpagar_id);
$remover->delete();
...
$cpagar_id 		= isset( $_POST['cpagar_id_cx'])		? $_POST['cpagar_id_cx']: 						$erro="O Campo Obrigatorio 1-0 Faltando.";
$historico		= isset( $_POST['historico_cx'])		? $_POST['historico_cx']:  						$erro="O Campo Obrigatorio 0-1 Faltando.";
$vlr_nominal_cx	= isset( $_POST['vlr_nominal_cx'])		? tool::limpamoney($_POST['vlr_nominal_cx']):	  	$erro="O Campo Obrigatorio 0-2 Faltando.";
$num_doc		= isset( $_POST['num_doc_cx'])			? $cpagar_id."-".$_POST['num_doc_cx']:	  		$erro="O Campo Obrigatorio 0-3 Faltando.";

$tp_lc			="1";
$tp 			="d";
$data 			=date("Y-m-d");

$formas_pagamentos_id 		= isset( $_POST['formas_pagamentos_id_cx'])		? $_POST['formas_pagamentos_id_cx']:  		$erro="O Campo Obrigatorio 0-4 Faltando.";
...
/* variavel de com o valor total que está sendo pago*/
$vlr_total_cx	= isset( $_POST['vlr_total_cx'])		? tool::limpamoney($_POST['vlr_total_cx']):	  	$erro="O Campo Obrigatorio 0-10 Faltando.";


$dt_pagamento				= isset( $_POST['dt_pagamento_cx'])	? tool::InvertDateTime(tool::LimpaString($_POST['dt_pagamento_cx']),"0"):    $erro="O Campo Obrigatorio 0-10 Faltando.";
...
if(tool::LimpaString($dt_pagamento) == ""){$erro ="Data de pagamento  invalida!";}
if(tool::LimpaString($dt_vencimento) == ""){$erro ="Data de vencimento  invalida!";}
...
// VERIFICAMOS SE NÃO HOUVE ERROS E RETORNA A MSG
	if(!$create or !$update){
		echo '":"","callback":"1","msg":"Houve um erro ao realizar o pagamento!","status":"warning';
	}else{
		echo '":"","callback":"0","msg":"Pagamento realizado com sucesso!","status":"success';
	}
