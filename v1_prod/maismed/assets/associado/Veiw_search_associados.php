...
// matricula
    ...
    if (is_numeric($_POST['vl'])) {

        $vl_campo = tool::limpaString($_POST['vl']);
        $where = "( matricula ='" . $vl_campo . "' or cod_mais_med= '" . $vl_campo . "')";
    }
// pesquisa pelo nome
...
// data de nascimento
    if (strstr($_POST['vl'], "/")) {
        ...
        if (strlen($_POST['vl']) < 10) {
            ...
            $vl_campo = tool::limpaString($_POST['vl']);
            $where = "dt_nascimento ='" . tool::InvertDateTime($vl_campo, 0) . "'";
        }
    ...
    $query = "SELECT SQL_CACHE
		...
		FROM associados WHERE " . $where . " ORDER BY matricula DESC";
    ...
    $query = "SELECT
		...
		(SELECT count(id) FROM faturamentos  WHERE status = '1' and matricula = associados.matricula) as total_fat_pgto,
		(SELECT count(id) FROM faturamentos  WHERE status = '2' and matricula = associados.matricula) as total_fat_cancel
		FROM associados ORDER BY matricula DESC LIMIT 5";
}
    ...
    <article class="uk-comment">
        <header class="uk-comment-header " style="padding: 5px;">
            ...
            <h4 class="uk-comment-title uk-text-bold">
                <?php
                echo $associados->current()->matricula." ".$associados->current()->nm_associado . " - [ " . $associados->current()->nm_convenio . " ]";
