...
// pesquisa pelo nome
if (ctype_alpha(tool::limpaString($_POST['vl']))) {

    $where = "dependentes.nome LIKE '" . $_POST['vl'] . "%'";

} elseif (is_numeric($_POST['vl'])) {

    $where = "dependentes.matricula ='" . tool::limpaString($_POST['vl']) . "'";

} else {
    $erro = 1;
    $msg = "Infelismente so possivel pesquisar dependentes pelo nome.";
...
$query = "SELECT SQL_CACHE
          ...
          associados.numprontuario,
		  associados.nm_associado,
          associados.contrato,
		  associados.convenios_id,
		  associados.dt_cadastro,
		  associados.cpf,
		  associados.dt_nascimento,
		  associados.status,
		  dependentes.nome as nm_dependente,
		...
		FROM
		  associados
		  LEFT JOIN dependentes ON associados.matricula = dependentes.matricula
		WHERE
		  " . $where . "";
    ...
    <article class="uk-comment">
        <header class="uk-comment-header " style="padding: 5px;">
            ...
            <h4 class="uk-comment-title uk-text-bold">
                <?php
                echo $associados->current()->matricula." ".$associados->current()->nm_dependente . " - [ " . $associados->current()->nm_convenio . " ]";
                    ...
                    <div data-uk-dropdown="{pos:'left-center',mode:'click'}">
                        ...
                        <div class="uk-dropdown uk-dropdown-small">
                            <ul class="uk-nav uk-nav-dropdown">
                                ...
                                <li><a href="JavaScript:void(0);" onclick="D_Actions_Assoc('fat','<?php echo $matricula; ?>','<?php echo $convenio; ?>');"><i class="uk-icon-list"></i> Faturamento</a>
                                </li>
                            </ul>
                        </div>
                    </div>
            ...
            <div class="uk-comment-meta uk-text-bold " style="height:30px;">
                Codigo: <?php echo tool::CompletaZeros(10, $associados->current()->cod_mais_med); ?> |
                CPF: <?php echo tool::MascaraCampos("###.###.###-##", $associados->current()->cpf); ?> |
                Data Cadastro: <?php $datacad = new ActiveRecord\DateTime($associados->current()->dt_cadastro);
                echo $datacad->format('d/m/Y'); ?> |
                Prontuario: <?php echo tool::CompletaZeros(10, $associados->current()->numprontuario); ?> |
                Usuário: <?php echo ucfirst($associados->current()->nm_usuario); ?>
            </div>
            <div class="uk-comment-meta uk-text-bold ">
                ...
                Dependentes:
                <div class="uk-badge uk-badge-success">Ativos <?php echo $associados->current()->total_dep; ?></div>
                Faturamento:
