...
$query_assoc = " SELECT count(matricula) as total_cancel,
        (SELECT count(matricula) FROM associados WHERE status ='1' and empresas_id='" . $COB_Empresa_Id . "') as total_ativos,
		...
		(SELECT count(id) FROM faturamentos  WHERE dt_vencimento between '" . $A . "-" . $M . "-01' and '" . $A . "-" . $M . "-31' and (status = 0 or status = 1) and tipo_parcela ='M' and empresas_id='" . $COB_Empresa_Id . "') as titulos_emitidos,
...
$query_inadimplentes = titulos::find_by_sql("SELECT SQL_CACHE
										...
										WHERE
									        faturamentos.referencia BETWEEN  '" . $prev_month11 . "' AND '" . $prev_month1 . "' AND faturamentos.status='0'
        ...
        <div class="uk-width-medium-1-4 uk-scrollspy-inview uk-animation-slide-top">
            <div class="uk-panel uk-panel-box uk-grid-dash" style="padding: 0; min-height: 150px; text-align: center;">
                ...
                <div class="uk-text-success uk-text-bold">
                    <?php echo $dadosassociado[0]->titulos_pgto . " R / " . $dadosassociado[0]->titulos_emitidos . " M = (" . number_format($indRecebiveis, 2, ".", "") . "%)"; ?>
                </div>
        ...
        <div class="uk-width-medium-1-4 k-scrollspy-inview uk-animation-slide-bottom">
            <div class="uk-panel uk-panel-box uk-grid-dash" style="padding: 0; min-height: 150px;  text-align: center;">
                ...
                <div class="uk-panel uk-badge-primary uk-text-center" style="bottom:0px; color: #fff; position: absolute; width: 94%; padding: 3%;">
                    Cadastros/Reativações - [<?php echo $M . "/" . $A; ?>] - [<?php echo $dadosassociado[0]->total_ativos; ?>]
                </div>
            </div>
        </div>
        <div class="uk-width-medium-1-4 k-scrollspy-inview uk-animation-slide-bottom">
                        ...
                        while ($list_parcelas->valid()):
                            ...
                            <tr style="line-height:23px;">
                                ...
                                <td class="uk-width uk-text-left"
                                    style="text-transform: capitalize;width:25px;max-width:25px;white-space:nowrap;text-overflow:ellipsis; overflow:hidden;"><?php echo strtolower($list_parcelas->current()->nm_associado); ?></td>
...
// data da ultima parcela paga
                                ...
                                <tr style="line-height:25px;">
                                    ...
                                    <td class="uk-width uk-text-center" style="width:100px;"><?php echo $list_inadimplentes->current()->fone_cel; ?></td>
...
<script type="text/javascript">
    ...
    jQuery(".uk-grid-dash a").click(function (event) {
        ...
        if (pg == "") {
            UIkit.modal.alert("Acesso ainda não liberado!");
            exit();
        }
