<div class="tabs-spacer" style="display:none;">
    ...
    $queryfatu=faturamentos::find_by_sql("SELECT SQL_CACHE
									  ...
									  (SELECT dados_cobranca.formascobranca_sys_id FROM dados_cobranca WHERE dados_cobranca.id = faturamentos.dados_cobranca_id AND dados_cobranca.status = 1 ) as iddadocobranca,
                                        planos.valor_dependente,
    ...
    $Depend = dependentes::find_by_sql("SELECT count(id) as total FROM dependentes WHERE matricula='".$FRM_matricula."' AND status='1'");
...
<div id="menu-float" style="text-align:center;margin:0 955px;top:42px;border:0;background-color:#546e7a;">
    ...
    <a id="Btn_fat_0" class="uk-icon-button uk-icon-archive" style="margin-top:2px;text-align:center;" data-uk-tooltip="{pos:'left'}" title="Dados de Cobrança" data-cached-title="Dados de Cobrança" ></a>
	...
	<?php if(intval($listfat[0]->iddadocobranca) == 1){ ?>
        ...
        <a id="Btn_fat_12" class="uk-icon-button uk-icon-file-text-o" style="margin-top:2px;text-align:center;" data-uk-tooltip="{pos:'left'}" title="Imprimir Carnê" data-cached-title="Imprimir Carnê" ></a>
    ...
    <?php }elseif(intval($listfat[0]->iddadocobranca) == 3){ ?>
        ...
        <a id="Btn_fat_4" class="uk-icon-button uk-icon-copy" style="margin-top:2px;text-align:center;" data-uk-modal="{target:'#DadosGeraTitulo'}" data-uk-tooltip="{pos:'left'}" title="Gerar Titulos" data-cached-title="Gerar Titulos" ></a>
        <a id="Btn_fat_10" class="uk-icon-button uk-icon-barcode" style="margin-top:2px;text-align:center;"   data-uk-tooltip="{pos:'left'}" title="Imprimir Titulos" data-cached-title="Imprimir Titulos" ></a>

    <?php } ?>
    <a id="Btn_fat_11" class="uk-icon-button uk-icon-clipboard" style="margin-top:2px;text-align:center;"   data-uk-tooltip="{pos:'left'}" title="Adcionar Procedimentos" data-cached-title="Adcionar Procedimentos" ></a>
    ...
    <nav class="uk-navbar">
        <table  class="uk-table" >
            ...
            <tr style="line-height: 30px;">
                ...
                <th class="uk-width uk-text-center" style="width:100px;"  data-uk-tooltip="" title="Codigo da parcela" data-cached-title="Codigo da parcela">Tipo</th>
                ...
                <th class="uk-width uk-text-center" style="width:100px;" data-uk-tooltip="" title="Banco Boleto" data-cached-title="Banco Boleto" >Ações</th>
    ...
    <div id="grid_faturamento" style="height:458px; overflow-y:scroll;">
        ...
        <table  class="uk-table uk-table-striped uk-table-hover" >
            ...
            while($listfat->valid()):
                ...
                $valor_atualizado = ($Depend[0]->total * $listfat->current()->valor_dependente) + $listfat->current()->valor_plano;

                $vlr_parcela = $listfat->current()->valor;

//                if(($listfat->current()->valor != $valor_atualizado && $ref->format('Ym') > date("Ym")) && $listfat->current()->tipo_parcela == "M"){
//
//                    $Query_update_fat=faturamentos::find_by_id_and_status($listfat->current()->id,0);
//
//                    $Query_update_fat->update_attributes(array("valor"=>"{$valor_atualizado}"));
//
//                    $vlr_parcela = $valor_atualizado;
//
//                }
...
//                        echo tool::CompletaZeros(10,$listfat->current()->id);
                        if($listfat->current()->tipo_parcela == "A"){echo '<div class="uk-badge uk-badge-warning">ADESÃO</div>';}else{echo '<div class="uk-badge uk-badge-success">MENSALIDADE</div>';}
                        ?>
                    </td>
                    <!--
                    <td class="uk-width uk-text-center" style="width:100px;">
       	...
       	-->
                    ...
                    <td class="uk-width uk-text-center" style="width:90px;"><?php if($listfat->current()->dt_pagamento=="" || $dtpgto->format('d/m/Y') == "01/01/0001"){echo"00/00/0000";}else{echo $dtpgto->format('d/m/Y');} ?></td>
                    <td class="uk-width uk-text-center uk-text-primary uk-text-bold" style="width:90px;">
                        <input type="text" autocomplete="off" style="width: 100%; height: 100%; border:0;  margin: 0; padding:0; background-color: transparent;" class="uk-text-center uk-vep" id="vep" value="<?php echo number_format($listfat->current()->valor,2,",","."); ?>" data-uk-valp="<?php echo number_format($listfat->current()->valor,2,",","."); ?>" readonly="readonly"/>
                    </td>
                    ...
                    <td lass="uk-width uk-text-center" style="width:93px;" >
                        ...
                        if($listfat->current()->carteira_bol == "CR"){echo' <div  class="uk-badge uk-badge-notification uk-text-small" data-uk-tooltip="" title="" data-cached-title="Titulo Registrado">R</div>';}
                        /* se a boleto for registrado exibimos uma informação infirmando ao usuario */
                        if($listfat->current()->total_pro > 0){echo' <div  class="uk-badge uk-badge-warning uk-badge-notification uk-text-small" data-uk-tooltip="" title="" data-cached-title="Parcela Prossui Procedimentos Médicos">P</div>';}

                        ?>
                    </td>
                    <th class="uk-width uk-text-center" style="width:20px;" >
                        <?php  if($listfat->current()->status== 0 ){ ?>
                            <input type="checkbox" style="margin:3px;" class="check_ab" name="check0[]" value="<?php echo $listfat->current()->id; ?>" data-uk-codbank="<?php echo $listfat->current()->cod_banco; ?>" data-uk-id-titulo="<?php echo $listfat->current()->titulos_bancarios_id; ?>" />
                        <?php }if($listfat->current()->status== 1 ){  ?>
                            <input type="checkbox" style="margin:3px;" class="check_pgto" name="check0[]" value="<?php echo $listfat->current()->id; ?>" data-uk-codbank="<?php echo $listfat->current()->cod_banco; ?>" data-uk-id-titulo="<?php echo $listfat->current()->titulos_bancarios_id; ?>" />
                        <?php }if($listfat->current()->status== 2 ){ ?>
                            <input type="checkbox" style="margin:3px;" class="check_cancel" name="check0[]" value="<?php echo $listfat->current()->id; ?>" data-uk-codbank="<?php echo $listfat->current()->cod_banco; ?>" data-uk-id-titulo="<?php echo $listfat->current()->titulos_bancarios_id; ?>" />
                        <?php }?>
                    </th>
                </tr>
                <?php
...
<script type="text/javascript" >
    ...
    jQuery("#grid_faturamento input[type=checkbox]").click(function(evento){
        ...
        if(checked == "check_ab"){
            ...
            if (jQuery(".check_ab").is(":checked")) {
                ...
                jQuery('.check_pgto,.check_cancel').each(function(){jQuery(this).attr("checked", false);});
            }else{
        ...
        if(checked == "check_pgto"){
            ...
            if (jQuery(".check_pgto").is(":checked")) {
                ...
                jQuery('.check_ab,.check_cancel').each(function(){jQuery(this).attr("checked", false);});
            }else{
                jQuery("#opt_pgto").css("display", "none");	// oculta o grupo de opções de estorno
        ...
        if(checked == "check_cancel"){
            ...
            if (jQuery(".check_cancel").is(":checked")) {
                ...
                jQuery('.check_pgto,.check_ab').each(function(){jQuery(this).attr("checked", false);});
    ...
    */
    jQuery("#Btn_fat_0").click(function(){
        ...
        New_window('file-text-o','500','260','Dados de Cobrança','assets/faturamento/Frm_dados_cobranca.php?matricula=<?php echo $FRM_matricula; ?>&convenio_id=<?php echo $FRM_convenio_id; ?>',true,false,'Carregando dados...');
    });
            ...
            jQuery("#grid_faturamento input[type=checkbox]").each(function(){
                ...
                if (this.checked) {
                    ...
                    if( jQuery.isEmptyObject(emissor_titulo)){
                        ...
                        if(jQuery(this).attr("data-uk-codbank") == 0){ UIkit.modal.alert("Parcela não possui titulo bancario!"); modal.hide();exit(); }
    ...
    jQuery(function(){
        ...
        jQuery("#Btn_fat_12").click(function(event) {
            ...
            if (ids.length === 0) {
                ...
                UIkit.modal.alert("Não existem parcelas selecionadas!"); modal.hide();exit();
            }
