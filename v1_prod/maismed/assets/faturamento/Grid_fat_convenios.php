...
<div class="tabs-spacer" style="display:none;">
    ...
    // contas bancarias habilitadas a emitir boleto
    $Query_contas1 = contas_bancarias::find_by_sql("SELECT id,nm_conta,cod_banco FROM contas_bancarias WHERE empresas_id='" . $COB_Empresa_Id . "' AND status='1' ORDER BY id");
    $List_contas1 = new ArrayIterator($Query_contas1);
    ...
    $queryfatu = faturamentos::find_by_sql("SELECT SQL_CACHE
                                                    ...
                                                    (SELECT sum(valor) FROM procedimentos 
                                                    WHERE procedimentos.valor > 0 and procedimentos.matricula  = associados.matricula and associados.status='0' and faturamentos_id = faturamentos.id 
                                                    GROUP BY faturamentos_id) as valor_pro,
                                                    
                                                    (SELECT count(id) FROM dependentes  d
                                                    inner join associados a on a.matricula = d.matricula
                                                    WHERE d.matricula  = associados.matricula and d.status='1') * p.valor_dependente as sum_dep,

                                                    (SELECT count(id) FROM dependentes  d
                                                    inner join associados a on a.matricula = d.matricula
                                                    WHERE d.matricula  = associados.matricula and d.status='1') as num_dep
                                                    
                                                    FROM faturamentos
                                                    inner JOIN associados ON associados.matricula = faturamentos.matricula
                                                    inner join dados_cobranca d on d.id = faturamentos.dados_cobranca_id
                                                    inner join planos p on p.id = d.planos_id
...
<!--    <a class="uk-icon-button uk-icon-copy" style="margin-top:2px;text-align:center;" data-uk-modal="{target:'#DadosGeraTitulo'}" data-uk-tooltip="{pos:'left'}" title="Gerar Titulo"-->
<!--       data-cached-title="Gerar Boleto"></a>-->
    <a class="uk-icon-button uk-icon-copy" style="margin-top:2px;text-align:center;" data-uk-modal="{target:'#DadosGeraCarne'}" data-uk-tooltip="{pos:'left'}" title="Gerar carnê"
       data-cached-title="Gerar carnê"></a>
    <a class="uk-icon-button uk-icon-floppy-o" style="margin-top:2px;text-align:center;" data-uk-modal="{target:'#DadosBaixaTitulo'}" data-uk-tooltip="{pos:'left'}" title="Baixar Parcelas"
       data-cached-title="Baixar Parcelas"></a>
...
<form class="uk-form uk-form-tab" id="FrmGridFatConvenios" style="padding: 0; margin: 0;">
    <nav class="uk-navbar">
        <table class="uk-table">
            ...
            <tr style="line-height: 25px;">
                <th class="uk-width uk-text-left" style="width:30px;">#</th>
                <th class="uk-width  uk-text-left" style="width:340px;" data-uk-tooltip="" title="Nome do Funcionario" data-cached-title="Nome do Funcionario">Nome Funcionario</th>
                <th class="uk-width uk-text-center" style="width:90px;" data-uk-tooltip="" title="Mês de referencia" data-cached-title="Mês de referencia">Referencia</th>
                ...
                <th class="uk-width uk-text-center" style="width:90px;" data-uk-tooltip="" title="Codigo da parcela" data-cached-title="Codigo da parcela">Dependentes</th>
    ...
    <div id="grid_faturamento_convenios" style="height:465px; overflow-y:scroll;">
        <table class="uk-table uk-table-striped uk-table-hover">
            ...
            while ($listfat->valid()):
                ...
                ?>
                <tr>
                    <th class="uk-width uk-text-center " style="width:30px;"><?=$lf;?></th>
                    <td class="uk-width  uk-text-left uk-text-bold" style="width:350px; text-transform: uppercase;"><?php echo $listfat->current()->nm_associado; ?></td>
                    <td class="uk-width uk-text-center uk-text-bold" style="width:95px;"><?php echo tool::Referencia($ref->format('Ymd'), "/"); ?></td>
                    <td class="uk-width uk-text-center" style="width:95px;"><?php echo $dtvenc->format('d/m/Y'); ?></td>
                    <td class="uk-width uk-text-center" style="width:95px;"><?php echo $listfat->current()->num_dep; ?></td>
                    <td class="uk-width uk-text-center" style="width:95px;">
                        <?php echo number_format($listfat->current()->valor, 2, ",", "."); ?>
                    </td>
                    <td colspan="2">
                        <input type="checkbox" style="margin-top: -5px;" checked="checked" disabled="disabled" class="check_ab" name="check0[]" value="<?php echo $listfat->current()->id; ?>"/>
                    </td>
                </tr>
                <?php
            ...
            <tr style="line-height: 30px; background-color: #f5f5f5;">
                <th class="uk-width  uk-text-left" style="width:100px;" colspan="2">Total</th>
                <th class="uk-width  uk-text-left" style="width:200px;" colspan="4"><?=$lf-1;?> Associados</th>
                <th class="uk-width uk-text-center uk-text-primary" style="width:90px;" colspan="3" data-uk-tooltip="" title="Valor da parcela" data-cached-title="Valor da parcela">
    ...
    <div id="DadosBaixaTitulo" class="uk-modal">
        <div class="uk-modal-dialog" style="width:500px;">
            <!--<button type="button" class="uk-modal-close uk-close"></button> -->
            <div class="uk-modal-header ">
                <h2><i class="uk-icon-bank"></i> Recebimento de convênio PJ</h2>
            </div>
            <label>
                <span style="width: 120px;">Selecione uma conta</span>
                <select name="conta_bancaria_pgto" id="conta_bancaria_pgto" class="select">
                    <?php
                    echo '<option value="0" selected="selected">Selecionar banco</option>';
                    while ($List_contas1->valid()):
                        echo '<option value="' . $List_contas1->current()->id . '">' . $List_contas1->current()->nm_conta . '</option>';
                        $List_contas1->next();
                    endwhile;
                    ?>
                </select>
            ...
            <label>
                <span style="width: 120px;">Data de pagamento</span>
                <input type="text" class="uk-text-center w_120" style="background-color: #fff; color: #000;" id="dt_titulo_pj_pgto" data-uk-datepicker="{format:'DD/MM/YYYY'}"/>
            </label>
            <label>
                <span>Forma Receb</span> <select name="f_receb" id="f_receb" class="select w_150">
                    ...
                    while ($formas->valid()) :
                        echo '<option value="' . $formas->current()->f_receb_emp_id . ';' . $formas->current()->acrescimos . ';' . $formas->current()->f_recebe_sys_id . '" >' . (strtoupper($formas->current()->descricao)) . '</option>';
                        $formas->next();
                    endwhile;
                    ?>
                </select>
                <div class="uk-modal-footer uk-text-right">
                    <a id="BtnPgto001" class="uk-button uk-button-primary uk-button-small"><i class="uk-icon-check"></i> Baixar</a>
    ...
    <div id="DadosGeraTitulo" class="uk-modal">
        <div class="uk-modal-dialog" style="width:500px;">
            ...
            <input type="hidden" readonly="readonly" value="<?php echo $vl_t; ?>" id="vl_t_title"/>
            <div class="uk-modal-footer uk-text-right">
    ...
    <div id="DadosGeraCarne" class="uk-modal">
        <div class="uk-modal-dialog" style="width:500px;">
            <!--<button type="button" class="uk-modal-close uk-close"></button> -->
            <div class="uk-modal-header ">
                <h2><i class="uk-icon-bank"></i> Dados do carnê</h2>
            </div>
            <label>
                <span>Vencimento</span>
                <input type="text" class="uk-text-center w_120" style="background-color: #fff; color: #000;" id="dt_venc_carne_pj" data-uk-datepicker="{format:'DD/MM/YYYY'}"/>
            </label>
            <input type="hidden" readonly="readonly" value="<?php echo $vl_t; ?>" id="vl_t_cn"/>
            <div class="uk-modal-footer uk-text-right">
                <a id="Btn_fat_10" class="uk-button uk-button-primary uk-button-small"><i class="uk-icon-check"></i> Confirmar</a>
                <a id="BtnCancelarFiltro" class="uk-button uk-button-danger uk-button-small uk-modal-close"><i class="uk-icon-remove"></i> Cancelar</a>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    */
    ...
    /* pega todas as parcelas selecionadas para geraçao do carne */
    jQuery(function () {
        jQuery("#BtnPgto001").click(function (event) {
            ...
            jQuery.ajax({
                async: true,
                url: "assets/faturamento/controllers/Controller_recebimento_pj.php",
                type: "POST",
                data: "ids=" + ids + "&pagamento=" + jQuery("#dt_titulo_pj_pgto").val() + "&conta_bancaria_id=" + jQuery("#conta_bancaria_pgto").val() + "&f_receb_id=" + str[0] + "&f_receb_sys_id=" + str[2]+"",
                success: function (resultado) {
                    var text = '{"' + resultado + '"}';
                    var obj = JSON.parse(text);
    ...
    */
    jQuery(function () {
        jQuery("#Btn_fat_10").click(function (event) {
            ...
            event.preventDefault();
            var LeftPosition = (screen.width) ? (screen.width - 980) / 2 : 0;
            window.open('<?php echo $caminho[0]; ?>assets/cobranca/carne/veiw_pdf_pj.php?ref=<?= tool::LimpaString($FRM_referencia); ?>&cid=<?= $FRM_convenio_id; ?>&vlr='+jQuery("#vl_t_cn").val()+'&vnc='+jQuery("#dt_venc_carne_pj").val()+'', 'Impressão', 'width=980, height=550, top=100, left=' + LeftPosition + ', scrollbars=yes, status=no, toolbar=no, location=no, directories=no, menubar=no, resizable=no, fullscreen=no');
