...
if ($FRM_acao == "update") {
    ...
    if ($Query_update->titulos_bancarios_id > 1) {
...
################################################################################################################################################################################
    ...
    if ($FRM_f_receb_sys_id == '001' or
        ...
        /*recebimento de boletos baixados no banco sem controle de emissão pelo sistema*/
    }elseif($FRM_f_receb_sys_id == '006'){

        $Select_conta = "SELECT id FROM contas_bancarias WHERE empresas_id='" . $COB_Empresa_Id . "' AND tp_conta='2' AND status='1'";
        ...
        if (!$Select_conta) {
            echo '":"","callback":"1","msg":"Empresa não possui conta configurada para recebimento de cartão.","status":"warning';
            exit();
        }
        ...
        if (!$Select_conta) {
            echo '":"","callback":"1","msg":"Empresa não possui conta configurada para recebimento de debito em conta.","status":"warning';
            exit();
        }
...
// definimos a variavel valor que será lançacado no caixa como zero antes do loop
    ...
    foreach ($FRM_pa as $value) {
        ...
        if (!$Query_update) {
            ...
            if ($Query_parcela[0]->titulos_bancarios_id > 1) {
