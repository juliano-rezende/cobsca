...
// não existe então vamos criar
    ...
    $last_dados_cobranca = dados_cobranca::find("last");
    // id do dado de cobrança gerado para o associado
    ...
    for ($i = 1; $i <= $Tparcelas; $i++) {
        ...
        $Depend = dependentes::find_by_sql("SELECT count(id) as total FROM dependentes WHERE matricula='1' AND status='1'");
        $vlr_depend = planos::find($FRM_planos_id);
        $vlr_parcela = ($Depend[0]->total * $vlr_depend->valor_dependente) + $last_dados_cobranca->valor;
