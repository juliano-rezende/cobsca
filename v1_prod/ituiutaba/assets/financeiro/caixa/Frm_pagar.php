...
$dadospagamento=caixa::find($_GET['id']);
}else{}
?>
</div>
<style type="text/css" media="screen">
#FrmPagamento{padding-top: 5px;}
#FrmPagamento span{width: 150px;}
</style>
<ul class="uk-tab uk-gradient-cinza" data-uk-tab style="height:35px;">
    <li ><a href="#Geral" >Geral</a></li>
    <li><a href="#Detalhes">Informações Adicionais</a></li>
</ul>
<form name="FrmPagamento" method="post" id="FrmPagamento" class="uk-form">
<div id="Geral" class="tab-content">

<label>
<span>Codigo</span>
<input value="<?php if(isset($dadospagamento)):echo $dadospagamento->id; endif; ?>"  name="id" type="text" class="uk-text-center w_100  " id="id" readonly="readonly" />
</label>
...
if(isset($dadospagamento)):
	...
	if($conta->current()->id == $dadospagamento->contas_bancarias_id){$select='selected="selected"';}else{$select="";}
...
$str 		=" ".$dadospagamento->clientes_fornecedores_id;
...
if($define_tp == 0){
	...
	echo'<option value="'.$dadospagamento->clientes_fornecedores_id.'" selected="selected">'.utf8_encode($receberdeAssoc->nm_associado).'</option>';
	...
	echo'<option value="1.'.$cli_for->current()->id.'">'.utf8_encode($cli_for->current()->id."-".$descricao).'</option>';
...
if($define_tp == 2){
		...
		echo'<option value="'.$dadospagamento->clientes_fornecedores_id.'" selected="selected">OUTROS CLIENTES OU FORNECEDORES</option>';
...
</select> <button class="uk-button uk-button-mini uk-button-success" type="button" id="Btn_pgto_005" disabled><i class="uk-icon-plus"></i></button>
</label>

<label>
<span>Plano de conta</span>
<select name="planos_contas_id" class="select w_350" id="planos_contas_id">
<?php
  if(isset($dadospagamento)):
  ...
  while($Arr_conta->valid()):
        ...
        echo'<optgroup label="'.utf8_encode(strtoupper($Arr_conta->current()->descricao)).'">';
          ...
          while($Arr_planos1->valid()):
              ...
              if($Arr_planos1->current()->id == $dadospagamento->planos_contas_id){$select='selected="selected"';}else{$select="";}
...
?>
</select> <button class="uk-button uk-button-mini uk-button-success" type="button" id="Btn_pgto_004" disabled><i class="uk-icon-plus"></i></button>
</label>
  ...
  while($Arr_c_custos->valid()):
        ...
        echo'<optgroup label="'.utf8_encode(strtoupper($Arr_c_custos->current()->descricao)).'">';
          ...
          while($Arr_c_custos1->valid()):
              ...
              if($Arr_c_custos1->current()->id == $dadospagamento->centros_custos_id){$select='selected="selected"';}else{$select="";}
...
</select> <button class="uk-button uk-button-mini uk-button-success" type="button" id="Btn_pgto_003" disabled><i class="uk-icon-plus"></i></button>
</label>

<label>
<span>F. pagamento</span>
<select name="formas_pagamentos_id" class="select w_350" id="formas_pagamentos_id">
<?php
	if(isset($dadospagamento)):

		$formas=formas_pagamentos::find_by_sql("SELECT formas_pagamento_sys.descricao,formas_pagamentos.id
			                                    FROM formas_pagamentos
			                                    LEFT JOIN formas_pagamento_sys ON formas_pagamento_sys.id = formas_pagamentos.formas_pagamento_sys_id
			                                    WHERE formas_pagamentos.status = '1' AND formas_pagamentos.empresas_id = '".$COB_Empresa_Id."'");
		$formas_list= new ArrayIterator($formas);
		while($formas_list->valid()):

		if($formas_list->current()->id == $dadospagamento->formas_recebimentos_id){$select='selected="selected"';}else{$select="";}
...
?>
</select> <button class="uk-button uk-button-mini uk-button-success" type="button" id="Btn_pgto_002" disabled><i class="uk-icon-plus"></i></button>

<label>
<span>Data</span>
<input name="data"  type="text" class=" w_100 uk-text-center " id="data" value="<?php
if(isset($dadospagamento)):   $now=new ActiveRecord\DateTime($dadospagamento->data);echo $now->format('d/m/Y'); else: echo date("d/m/Y"); endif ; ?> " data-uk-datepicker="{format:'DD/MM/YYYY'}" />
</label>

<label>
<span>Numero doc</span>
<input name="numdoc"  type="text" class=" w_100 uk-text-left " id="numdoc" value="<?php
if(isset($dadospagamento)):   echo $dadospagamento->numdoc; endif ; ?> " />
</label>

<label>
<span>Valor</span>
<input name="valor"  type="text" class=" w_100 uk-text-center " id="valor"  value="<?php
if(isset($dadospagamento)):   echo number_format($dadospagamento->valor,2,",","."); else: echo"0,00";endif ; ?>" />
</label>
</div>
<div id="Detalhes" class="tab-content" style="padding-top: 15px;">
<label>
<span>Historico</span>
<textarea name="historico" class="message" style="height:100px;" id="historico"><?php if(isset($dadospagamento)):   echo $dadospagamento->historico; endif ; ?></textarea>
</label>

<label>
<span>Detalhes</span>
<textarea name="detalhes" class="message" style="height:150px;" id="detalhes"><?php if(isset($dadospagamento)):   echo $dadospagamento->detalhes; endif ; ?> </textarea>
</label>
</div>
</form>
...
<div class="uk-button-group group_button"  style="border:0px solid #ccc;float:right; margin-top: 0px; ">
        ...
        <a href="JavaScript:void(0);" id="Btn_pgto_001" class="uk-button uk-button-primary  uk-button-small " style="border-left:1px solid #ccc;padding-top:2px;line-height: 28px;" ><i class="uk-icon-check uk-text-small " ></i> Confirmar</a>
<?php if(isset($dadospagamento)):?>
        <a href="JavaScript:void(0);" id="Btn_pgto_001" class="uk-button uk-button-danger  uk-button-small " onClick="FcExluir('<?php echo $_GET['id'];?>');" style="border-left:1px solid #ccc;padding-top:2px;line-height: 28px;" ><i class="uk-icon-remove uk-text-small" ></i> Remover</a>
<?php endif;?>
</div>
...
jQuery(function() {
	...
	jQuery("#Btn_pgto_001").click(function(event) {
		...
		if(historico==""){
					UIkit.modal.alert('Digite o Histórico do pagamento!');
					jQuery("#Detalhes").fadeIn();
					jQuery("#Geral").fadeOut();
					 return false;
			}
		if(valor==""){
					UIkit.modal.alert('Digite o Valor do pagamento!');
					 return false;
			}
					...
					event.preventDefault();
						...
						jQuery.ajax({
							url: "assets/financeiro/caixa/Controller_pagar.php",
							type: "post",
							data:'action=new_up&'+jQuery("#FrmPagamento").serialize(),
...
//faz a requisição
jQuery.post("assets/financeiro/caixa/Controller_pagar.php",
