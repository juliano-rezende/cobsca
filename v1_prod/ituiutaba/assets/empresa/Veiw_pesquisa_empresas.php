...
if(isset($_POST['acao'])){
	...
	if(empty ($_POST['vl'])){
	    $query="SELECT * FROM empresas WHERE id < '21'  ORDER BY id asc";
	// retorna a pesquisa pelo id enviado
	}elseif(is_numeric ($_POST['vl'])){
	    $query="SELECT * FROM empresas WHERE id ='".tool::limpaString($_POST['vl'])."'"; // pesquisa pelo id do cliente
	}elseif (ctype_alpha(tool::limpaString($_POST['vl']))) {
	    $query="SELECT *
				FROM  empresas
				WHERE (razao_social LIKE '%".tool::limpaString($_POST['vl'])."%' OR  (nm_fantasia like '%".tool::limpaString($_POST['vl'])."%')) ORDER BY id asc"; // pesquisa pela razao social ou nome fantasia
		...
		if ( !$cpf_cnpj->valida() ) {
		    ...
		    $query="SELECT * FROM empresas WHERE cnpj='".tool::limpaString($_POST['vl'])."'"; // pesquisa pelo cnpj da empresa fornecedora
		}
...
// corrigi bug do active record
echo'<div class="tabs-spacer" style="display:none;">';
$query_a=empresas::find_by_sql($query);
echo'</div>';
if(!$query_a){
	$erro	=1;
...
$empresas= new ArrayIterator($query_a);
while($empresas->valid()):
?>
    ...
    <article class="uk-comment">
        <header class="uk-comment-header ">
            ...
            <h4 class="uk-comment-title uk-text-bold">
				 <?php
					 echo utf8_encode($empresas->current()->nm_fantasia);
				 ?>
            </h4>
					...
					<div data-uk-dropdown="{pos:'left-center',mode:'click'}">
						...
						<div class="uk-dropdown uk-dropdown-small" >
							<ul class="uk-nav uk-nav-dropdown">
								<li><a  onclick="D_Actions_Empresas('edit','<?php echo $empresas->current()->id;?>',null);" data-uk-tooltip="{pos:'left'}"><i class="uk-icon-edit"></i> Editar/Visualizar</a></li>
							</ul>
						</div>
            ...
            <div class="uk-comment-meta uk-text-bold "  style="height:30px;">
                Codigo: <?php echo tool::CompletaZeros(10,$empresas->current()->id); ?> |
                CNPJ: <?php echo tool::MascaraCampos("??.???.???/????-??",$empresas->current()->cnpj); ?> |
                Data Cadastro: <?php  $datacad= new ActiveRecord\DateTime($empresas->current()->dt_cadastro);echo $datacad->format('d/m/Y');
...
function D_Actions_Empresas(action,val){

/* abre o dependente em modo de edição */
if(action=='edit'){

	LoadContent('assets/empresa/Frm_empresa.php?emp_id='+val+'','content');
