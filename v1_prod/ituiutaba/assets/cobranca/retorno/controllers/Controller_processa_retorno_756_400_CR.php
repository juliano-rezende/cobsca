...
$lote_interno       = substr($linha,95,4).substr(date("Y"),0,2).substr($linha,99,2).date("his"); //Lote de serviço - Número seqüencial para identificar um lote de serviço. DDMMYY de geração do arquivo mais numero
...
if(($titulo[0]->status == 1 && $d_cod_mov!= 10) || $titulo[0]->status == 1 && $d_cod_mov!= 6){
  ...
  $dt_pgto  = new ActiveRecord\DateTime($titulo->dt_pagamento);
  ...
  /*********************************************************** SE POSSUI SEGURO FAZ A INSERÇÃO DO DADOS NA TABELA DE SEGURADOS *************************/
      ...
      if($list_fat->current()->seguro == 1){
       ...
       // passa a matricula a referencia e o tipo de convenio pj ou pf
        ...
        if($Query_assegurar == true){$msg_pro.=" | Associado assegurado!";}
      }
...
if($tipo_reg == 9 or $tipo_reg == ""){ continue; }
...
//diretorio
$caminho ="../logs/emp_".$COB_Empresa_Id."_bank_".$Query_retorno->cod_banco."";
...
$arquivo=$caminho."/".$filename_log;
    ...
    if($alterados > 0){ echo "<hr><div class='uk-alert uk-alert-warning' style='margin-bottom:0px;'> Titulos baixados ".$alterados."</div></br>"; }
    if($baixados > 0){ echo "<div class='uk-alert uk-alert-warning' style='margin-bottom:0px;'> Titulos baixados ".$baixados."</div></br>"; }
    if($cancelados > 0){ echo "<div class='uk-alert uk-alert-warning' style='margin:0;'> Titulos baixados pelo banco  ".$cancelados."</div></br>"; }
    if($compensados > 0){ echo "<div class='uk-alert uk-alert-success' style='margin-bottom:0px;'> Titulos Compensados ".$compensados."</div></br>"; }

}else{

  if($alterados > 0){ echo "<div class='uk-alert uk-alert-warning' style='margin-bottom:0px;'> Titulos baixados ".$alterados."</div></br>";}
  if($baixados > 0){ echo "<div class='uk-alert uk-alert-warning' style='margin-bottom:0px;'> Titulos baixados ".$baixados."</div></br>";}
  if($cancelados > 0){ echo "<div class='uk-alert uk-alert-warning' style='margin:0;'> Titulos baixados pelo banco  ".$cancelados."</div></br>"; }
  if($compensados > 0){ echo "<div class='uk-alert uk-alert-success' style='margin-bottom:0px;'> Titulos Compensados ".$compensados."</div></br>";}
