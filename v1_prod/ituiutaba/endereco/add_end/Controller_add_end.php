...
if($FRM_acao == 'cidade'){
    ...
    $findCidades = cidades::find_by_sql("SELECT descricao,id FROM cidades WHERE estados_id='" . $FRM_uf_id . "' AND descricao LIKE '".$FRM_cidade."%'");

	if ($findCidades) {
	    
	    $opt = '<option selected="selected" value="">Selecione a Cidade</option>';
	    foreach($findCidades as $cidade){
	        $opt .= '<option value="' . $cidade->id . '">' . utf8_encode($cidade->descricao) . '</option>';
	    }
...
if($FRM_acao == 'bairro'){
    ...
    $findBairros = bairros::find_by_sql("SELECT descricao,id FROM bairros WHERE estados_id='" . $FRM_uf_id . "' AND cidades_id = $FRM_cidade_id AND descricao LIKE '".$FRM_bairro."%'");

	if ($findBairros) {
	    
	    $opt = '<option selected="selected" value="">Selecione um Bairro</option>';
	    foreach($findBairros as $bairro){
	        $opt .= '<option value="' . $bairro->id . '">' . utf8_encode($bairro->descricao) . '</option>';
	    }
...
$findLogradouros = bairros::find_by_sql("SELECT descricao,id FROM logradouros WHERE estados_id='" . $FRM_uf_id . "' AND cidades_id = $FRM_cidade_id AND bairros_id = $FRM_bairro_id AND descricao LIKE '".$FRM_logradouro."%'");



	if ($findLogradouros) {
	    
	    $opt = '<option selected="selected" value="">Selecione um Logradouro</option>';
	    foreach($findLogradouros as $logradouro){
	        $opt .= '<option value="'.$logradouro->id.'">'.$logradouro->complemento." ".utf8_encode($logradouro->descricao).'</option>';
	    }
