...
$FRM_convenio       = $_POST['convenio_id'];
$FRM_dt_inicial     = $_POST['dtini'];
$FRM_dt_final       =$_POST['dtfim'];
$FRM_ano_pesquisa  =$_POST['ano_pesquisa'];
$erro = "";
...
$limite = 10; // Define o limite de registros a serem exibidos com o valor cinco

$inicio = ceil(($pagina * $limite) - $limite);


if (($FRM_dt_inicial == "" or $FRM_dt_final == "") & $FRM_ano_pesquisa == "") {
...
*/
if ($FRM_convenio != "") {
  $convenio = " AND faturamentos.convenios_id='" . $FRM_convenio . "'";
} else {
  $convenio = "";
}


if($FRM_ano_pesquisa > 0){
  
  $hoje = date("Y-m-d");
  $where = " faturamentos.status='0' AND faturamentos.dt_vencimento < '{$hoje}' AND YEAR(associados.dt_cadastro) = '{$FRM_ano_pesquisa}' {$convenio} group by faturamentos.matricula";
  
}else{
  
  $where = " faturamentos.status='0' AND faturamentos.dt_vencimento BETWEEN  '" . tool::InvertDateTime(tool::LimpaString($FRM_dt_inicial), 0) . "' AND '" . tool::InvertDateTime(tool::LimpaString($FRM_dt_final), 0) . "' " . $convenio . " group by faturamentos.matricula";
...
<div class="tabs-spacer" style="display:none;">
  ...
  $query = "SELECT
  faturamentos.*,associados.nm_associado,associados.fone_cel,associados.dt_cadastro,associados.fone_fixo,associados.convenios_id AS cdconv,
  ...
  (SELECT sum(faturamentos.valor) FROM faturamentos  WHERE faturamentos.status = '0' and faturamentos.matricula = associados.matricula) as total_amount_ab,
...
<div style="height:<?php echo tool::HeightContent($COB_Heigth, $COB_Browser) - 75; ?>px; overflow:hidden; padding-bottom: 55px; ">
    ...
    <table class="uk-table uk-table-striped uk-table-hover">
        ...
        while ($Listitles->valid()):
          ...
          $dt_cadastro = new ActiveRecord\DateTime($Listitles->current()->dt_cadastro);
          ...
          ?>
            <tr style="line-height:22px;" class="<?php echo $class; ?>">
                ...
                <td class="uk-width uk-text-center" style="width:90px;"><?php echo $Listitles->current()->matricula; ?></td>
                <td class="uk-width uk-text-center" style="width:90px;"><?php echo $dt_cadastro->format('d/m/Y'); ?></td>
                <td class="uk-text-left"
                    style="text-transform: uppercase;"><?php echo utf8_encode($Listitles->current()->nm_associado); ?></td>
                <td class="uk-width uk-text-center"
                    style="width:100px;"><?php echo $Listitles->current()->fone_cel; ?></td>
                <td class="uk-width uk-text-center "
                    style="width:100px;"><?php echo $Listitles->current()->fone_fixo; ?></td>
                <td class="uk-width uk-text-center"
                    style="width:100px;"><?php echo number_format($Listitles->current()->total_amount_ab, 2, ",", "."); ?></td>
                <td class="uk-width uk-text-center" style="width:150px;">
                    <div class="uk-badge uk-badge-success" style="width:100px;">
                        Pagas <?php echo $Listitles->current()->total_fat_pgto; ?></div>
                    <div class="uk-badge uk-badge-warning" style="width:100px;">Em
                        aberto <?php echo $Listitles->current()->total_fat_ab; ?></div>
                    <div class="uk-badge uk-badge-danger" style="width:100px;">
                        Canceladas <?php echo $Listitles->current()->total_fat_cancel; ?></div>
                    <div class="uk-badge uk-badge-primary" style="width:100px;">
                        Total <?php echo $Listitles->current()->total_fat; ?></div>
                </td>
                <td class="uk-width uk-text-center" style="width:120px; vertical-align: middle;">
                    <div class="uk-coment-action"
                         style="font-weight: normal; text-align: left; z-index: 10000; position: absolute; margin: -20px 25px;">
                        <div class="uk-button-group">
                            <div data-uk-dropdown="{pos:'left-center',mode:'click'}">
                                <button class=" uk-button uk-button uk-button-large uk-icon-ellipsis-v "
                                        style="margin:5px 0; border:0; background:none;"></button>
                                <div class="uk-dropdown uk-dropdown-small">
                                    <ul class="uk-nav uk-nav-dropdown">
                                        <li><a href="javascript:void(0);" class="BtnHist"
                                               data-mat="<?php echo $Listitles->current()->matricula; ?>"><i
                                                        class="uk-icon-edit"></i> Adcionar Histórico</a></li>
                                        <li><a href="JavaScript:void(0);"
                                               onclick="D_Actions_Assoc('fat','<?php echo $Listitles->current()->matricula; ?>','<?php echo $Listitles->current()->cdconv; ?>');"><i
                                                        class="uk-icon-list"></i> Faturamento</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
...
<nav class="uk-navbar " style="bottom: 0; position: absolute; width: 100%">
    <span style="float: left; margin: 7px;"
          class="uk-text-small"><?php echo "Total de Registros " . $items . ""; ?></span>
    <ul class="uk-pagination" style="position:relative; margin:3px 0;"
        data-uk-pagination="{edges:4,items:<?php echo $items; ?>, itemsOnPage:10, currentPage:<?php echo $pagina; ?>}"></ul>
