...
if ($FRM_acao == "aceite_protesto") {

    $id = isset($_POST['id']) ? $_POST['id'] : tool::msg_erros("O Campo id é Obrigatorio.");

    // recuperamos o motivo do cancelamento
    $query_motivo = protestos::find($id);
    $query_motivo->update_attributes(array('aceite_adv' =>"S"));

    // VERIFICAMOS SE NÃO HOUVE ERROS E RETORNA A MSG
    if (!$query_motivo) {
        echo '":"","callback":"1","msg":"Erro ao aceitar protesto, contate o suporte","status":"warning';
    } else {
        echo '":"","callback":"0","msg":"Protesto aceito com sucesso!.","status":"success';
    }
...
if(!$Query_create ){
		$query_errors.=("<div class='uk-alert uk-alert-danger'><i class='uk-icon-warning  uk-text-danger' ></i> Erro ao criar historico do cancelamento.</br></div>");
}else{
	...
	if(!$update_fat ){
			$query_errors.=("<div class='uk-alert uk-alert-danger'><i class='uk-icon-warning  uk-text-danger' ></i> Erro ao cancelar parcela nº ".$list->current()->id.".</br></div>");
	}else{
		...
		if($update_titulo->cod_remessa > '0'){
			...
			$update_titulo->update_attributes(
				...
				if(!$update_titulo){
					$query_errors.=("<div class='uk-alert uk-alert-danger'><i class='uk-icon-warning  uk-text-danger' ></i> Erro ao cancelar titulo nº ".$list->current()->titulos_bancarios_id.".</br></div>");
				}
		}
		}
...
}elseif($FRM_acao == "remove_protesto"){


    $FRM_id	=	isset( $_POST['id']) 		? $_POST['id']			: tool::msg_erros("O Campo id é Obrigatorio.");

    $remover=protestos::find($FRM_id);
    $remover->update_attributes(array('status'=>3,'deleted_at'=>date("Y-m-d h:i:s")));
