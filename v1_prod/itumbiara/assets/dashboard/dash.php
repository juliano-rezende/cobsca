...
<script type="text/javascript" src="framework/uikit-2.24.0/js/core/scrollspy.min.js"></script>
...
$query_assoc = " SELECT count(matricula) as total_cancel,
                    ...
                    (SELECT count(id) FROM faturamentos  WHERE status ='0' and dt_vencimento between '" . $A . "-" . $M . "-01' and '" . $A . "-" . $M . "-31' and empresas_id='" . $COB_Empresa_Id . "') as titulos_abertos
...
$next_month = date('Y-m-d', strtotime('+1 months', strtotime(date('Y-m') . "-01")));
$ini2_month = date('Y-m-d', strtotime('-1 months', strtotime(date('Y-m') . "-01")));
...
$query_reserva = faturamentos::find_by_sql("SELECT SQL_CACHE
                                                    ...
                                                    WHERE
                                                    m.matricula = faturamentos.matricula  AND  (m.referencia = '" . $next_month . "') AND m.status < 3
                                                    )
                                                    GROUP BY
                                                    faturamentos.matricula
                                                    ORDER BY
...
$query_inadimplentes = titulos::find_by_sql("SELECT SQL_CACHE
                                                        ...
                                                        faturamentos.status='0' AND
                                                        faturamentos.dt_vencimento < '" . date("Y-m-d") . "'
...
$query_protesto = faturamentos::find_by_sql("SELECT SQL_CACHE
                                                    protestos.*,
                                                    faturamentos.matricula,
                                                    associados.nm_associado,
                                                    associados.cpf,
                                                    ...
                                                    associados.fone_fixo,
                                                    associados.fone_trabalho,
                                                    (SELECT sum(valor) FROM faturamentos WHERE matricula = associados.matricula AND faturamentos.status = '0' AND faturamentos.valor) as valor_ab,
                                                    (SELECT max(referencia) FROM faturamentos WHERE matricula = associados.matricula AND faturamentos.status < '3') as referencia,
                                                    protestos.status,protestos.id as protid
                                                    FROM
                                                    faturamentos
                                                    LEFT JOIN associados ON associados.matricula = faturamentos.matricula
                                                    LEFT JOIN convenios ON convenios.id = associados.convenios_id
                                                    LEFT JOIN protestos ON protestos.matricula = associados.matricula
                                                    WHERE
                                                    associados.status = '1' AND faturamentos.status  !='3' AND protestos.`status` < 2 AND protestos.status = 0
                                                    GROUP BY 
                                                    faturamentos.matricula
                                                    ORDER BY
                                                    associados.convenios_id ASC ,faturamentos.matricula ASC");
$list_protesto = new ArrayIterator($query_protesto);
...
<div id="big_col_0" style="margin: 0 auto;">
    ...
    <?php
    if ($COB_Acesso_Id == 4 || $COB_Acesso_Id == 3){
    ?>
                        ...
                        <table class="uk-table uk-table-striped uk-table-hover" style="border-top: 0px solid #ccc;">
                            ...
                            while ($list_parcelas->valid()):
                                ...
                                <tr style="line-height:23px;">
                                    ...
                                    <td class="uk-width uk-text-left"
                                        style="text-transform: capitalize;width:175px;max-width:175px;white-space:nowrap;text-overflow:ellipsis; overflow:hidden;"><?php echo strtolower($list_parcelas->current()->nm_associado); ?></td>
                            ...
                            if (count($Q_notificacoes) > 0) {
                                while ($list_notif->valid()):
                                    ...
                                    <h3 class="uk-accordion-title uk-text-muted <?php echo notificacoes::Status_msg($list_notif->current()->indice, $list_notif->current()->status); ?> uk-text-small">
                                        ...
                                        <i class="uk-icon-plus uk-text-muted uk-icon-toggle" style="float:right; margin-top: 2px;" uk-data-st="<?php echo $list_notif->current()->status; ?>"
                                           uk-data-id="<?php echo $list_notif->current()->id; ?>"></i>
                                    </h3>
        ...
        <div class="uk-width-medium-1-9 uk-row-first">
            <div class="uk-panel uk-panel-box">
                <h3 class="uk-panel-title uk-text-danger">
                    <i class="uk-icon-users"></i> Associados setor juridico
                    <i class="uk-icon-print uk-icon-small" onclick="Print('big_col_2_grid');" style="cursor: pointer; float: right;" data-uk-tooltip="" title=""
                       data-cached-title="Imprimir listagem"></i>
                </h3>
                <div class="uk-panel uk-text-left" style="height: 300px; overflow-y:none; width:100%; padding-right: 3px; ">
                    <table class="uk-table" style="border-top: 1px solid #ccc;">
                        ...
                        <tr style="line-height:25px;">
                            ...
                            <th class="uk-width uk-text-center" style="width:100px;">Residencia</th>
                            <th class="uk-width uk-text-center" style="width:100px;">Trabalho</th>
                            <th class="uk-width uk-text-center" style="width:100px;">celular</th>
                            <th class="uk-width uk-text-center" style="width:140px;">Ult Parcela Paga</th>
                            <th class="uk-width uk-text-center" style="width:140px;">Valor em atraso</th>
                    ...
                    <div class="uk-panel uk-text-left" id="big_col_2_grid" style="height: 275px; overflow-y: auto; width:100%;">
                        <table class="uk-table uk-table-striped uk-table-hover" style="border-top: 0px solid #ccc;font-size: 11px;" id="protestos">
                            <tbody>
                            <?php
                            $c = 1;
                            while ($list_protesto->valid()):
                                if (strlen(tool::LimpaString($list_protesto->current()->fone_cel)) == "10") {
                                    $fone_cel = tool::MascaraCampos("(??) ????? ????", substr(tool::LimpaString($list_protesto->current()->fone_cel), 0, 2) . "0" . substr(tool::LimpaString($list_protesto->current()->fone_cel), 2, 8));
                                } else {
                                    $fone_cel = tool::MascaraCampos("(??) ????? ????", substr(tool::LimpaString($list_protesto->current()->fone_cel), 0, 2) . " " . substr(tool::LimpaString($list_protesto->current()->fone_cel), 2, 8));
                                }
                                $data_parcela = new ActiveRecord\DateTime($list_protesto->current()->referencia);

                                if($list_protesto->current()->status == 0){
                                    $st = '<div class="uk-badge uk-badge-primary">Em analise</div>';
                                }if($list_protesto->current()->status == 1){
                                $st = '<div class="uk-badge uk-badge-danger">Protestado</div>';
                            }
                                ?>
                                <tr style="line-height:25px;">
                                    ...
                                    <td class="uk-width uk-text-center" style="width:100px;"><?php echo $list_protesto->current()->matricula; ?></td>
                                    <td class="uk-width uk-text-left uk-text-uppercase uk-text-overflow" style="width:310px;max-width: 310px;">
                                        <?php echo strtoupper($list_protesto->current()->nm_associado); ?>
                                    </td>
                                    <td class="uk-width uk-text-center" style="width:100px;"><?php echo tool::MascaraCampos("(??) ???? ????", $list_protesto->current()->fone_trabalho); ?></td>
                                    <td class="uk-width uk-text-center" style="width:100px;"><?php echo tool::MascaraCampos("(??) ???? ????", $list_protesto->current()->fone_fixo); ?></td>
                                    <td class="uk-width uk-text-center" style="width:100px;"><?php echo $fone_cel; ?></td>
                                    <td class="uk-width uk-text-center" style="width:140px;"><?php echo $data_parcela->format('m/Y'); ?></td>
                                    <td class="uk-width uk-text-center" style="width:140px;"><?php echo number_format($list_protesto->current()->valor_ab,2,",","."); ?></td>
                                    <td class="uk-width uk-text-left  uk-text-uppercase uk-text-overflow"
                                        style="width:200px;max-width: 200px;"><?php echo strtoupper($list_protesto->current()->razao_social); ?>
                                    </td>
                                    <td style="text-align: right; padding-right: 10px;">
                                        <?php
                                        if($list_protesto->current()->aceite_adv == 'S'):
                                            ?>
                                            <button class="uk-button" title="Listagem de faturamento" onclick="viewAssociado('<?php echo $list_protesto->current()->matricula; ?>');"><i class="uk-icon-list"></i></button>
                                            <button class="uk-button" title="historico do cliente" onclick="GridHist('<?php echo $list_protesto->current()->protid; ?>','<?php echo $list_protesto->current()->matricula; ?>');" ><i class="uk-icon-eye"></i></button>
                                            <button class="uk-button uk-button-danger" title="Remover Protesto" data-action="remove" data-id="<?=$list_protesto->current()->id;?>"><i class="uk-icon-remove"></i></button>
                                        <?php
                                        else:
                                            ?>
                                              <button class="uk-button uk-button-primary" onclick="aceiteProtesto('<?php echo $list_protesto->current()->protid; ?>')"><i class="uk-icon-search"></i> Aceitar</button>
                                ...
                                <?php $c++;
                                $list_protesto->next(); endwhile; ?>
                            </tbody>
                        </table>
                        <script>
                            function GridHist(id,mat){
                            
                                New_window('archive','800','500','Historicos da cobrança','assets/associado/Grid_cobranca.php?id='+id+'&mat='+mat+'',true,false,'Carregando...');
                            }
                            
                             /* botao de atalho para visulizar o cadastro do associado*/
                            function aceiteProtesto(id){

                                /*desabilita o envento padrao do formulario*/
                                event.preventDefault();
                                
                                 var data = "action=aceite_protesto&id=" + id + "";
            ...
            <div class="uk-width-medium-1-9 uk-row-first">
                <div class="uk-panel uk-panel-box">
                    <h3 class="uk-panel-title uk-text-primary">
                        ...
                        <i class="uk-icon-print uk-icon-small" onclick="Print('big_col_2_grid_emissao_carne');" style="cursor: pointer; float: right;" data-uk-tooltip="" title=""
                           data-cached-title="Imprimir listagem"></i>
                    </h3>
                    <div class="uk-panel uk-text-left" style="height: 300px; overflow-y:none; width:100%; padding-right: 3px; ">
                        <table class="uk-table" style="border-top: 1px solid #ccc;">
                        ...
                        <div class="uk-panel uk-text-left" id="big_col_2_grid_emissao_carne" style="height: 275px; overflow-y: auto; width:100%;">
                            <table class="uk-table uk-table-striped uk-table-hover" style="border-top: 0px solid #ccc;font-size: 11px;">
                                ...
                                while ($list_reserva->valid()):
                                    ...
                                    <tr style="line-height:25px;">
                                        ...
                                        <td class="uk-width uk-text-left  uk-text-uppercase uk-text-overflow"
                                            style="width:200px;max-width: 200px;"><?php echo strtoupper($list_reserva->current()->razao_social); ?></td>
                                        <td style="text-align: right; padding-right: 10px;">
                                            <a onclick="viewAssociado('<?php echo $list_reserva->current()->matricula; ?>');"><i class="uk-icon-search uk-icon-small "></i></a>
                                        </td>
                                    </tr>
                                    <?php $c++;
                                    $list_reserva->next(); endwhile; ?>
                                </tbody>
                            </table>
                        </div>
                    </div>
            ...
            <div class="uk-width-medium-1-9 uk-row-first">
                <div class="uk-panel uk-panel-box">
                    <h3 class="uk-panel-title uk-text-danger">
                        ...
                        <i class="uk-icon-print uk-icon-small " onclick="Print('big_col_3_grid');" style="cursor: pointer; float: right;" data-uk-tooltip="" title=""
                           data-cached-title="Imprimir listagem"></i>
                    </h3>
                    <div class="uk-panel uk-text-left" style="height: 300px; overflow-y:none; width:100%; padding-right: 3px; ">
                        <table class="uk-table" style="border-top: 1px solid #ccc;">
...
// data da ultima parcela paga
                                    ...
                                    if ($list_inadimplentes->current()->ult_pgto == "") {
                                        $data_parcela = "-";
                                    } else {
                                        $data_ref = new ActiveRecord\DateTime($list_inadimplentes->current()->ult_pgto);
                                        $data_parcela = $data_ref->format('m/Y');
                                    }
                                    ?>
                                    <tr style="line-height:25px;">
                                        <th class="uk-width uk-text-center" style="width:20px;">
                                        ...
                                        <td class="uk-width uk-text-center" style="width:150px;"><?php echo $data_parcela; ?></td>
                                        <td class="uk-width uk-text-center" style="width:100px;"><?php echo $fone_cel; ?></td>
                                        ...
                                        <td class="uk-width uk-text-left  uk-text-uppercase uk-text-overflow"
                                            style="width:200px;max-width: 200px;"><?php echo $list_inadimplentes->current()->razao_social; ?></td>
                                        <td style="text-align: right; padding-right: 10px;">
                                            <a onclick="viewAssociado('<?php echo $list_inadimplentes->current()->matricula; ?>','<?php echo $list_inadimplentes->current()->convenios_id; ?>');"><i
                                                        class="uk-icon-search uk-icon-small "></i></a>
                                        </td>
                                    </tr>
                                    <?php $inad++;
                                    $list_inadimplentes->next(); endwhile; ?>
                                </tbody>
                            </table>
                        </div>
                    </div>
    ...
    <!-- /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////// -->
    <div id="big_col_6" class="uk-scrollpsy uk-width-1-1 uk-margin-bottom" style="height:390px; background-color: #f5f5f5;">
        <div class="uk-grid uk-width" style="padding:10px; width: 90%; margin: 0 auto;  ">
            <div class="uk-width-medium-1-9 uk-row-first">
                <div class="uk-panel uk-panel-box">
                    <h3 class="uk-panel-title uk-text-primary">
                        <i class="uk-icon-users" style="cursor: pointer;"></i> Assegurados no mês
                        <i class="uk-icon-print uk-icon-smal " onclick="Print('big_col_6_grid');" style="cursor: pointer; float: right; margin-left: 4px;" data-uk-tooltip="" title=""
                           data-cached-title="Imprimir "></i>
                        <i class="uk-icon-file-zip-o uk-icon-small " id="Btn_excel_seg" style="cursor: pointer; float: right;" data-uk-tooltip="" title="" data-cached-title="Excel zipado"></i>
                    </h3>
                    <div class="uk-panel uk-text-left" style="height: 300px; overflow-y:none; width:100%; padding-right: 3px; ">
                        <table class="uk-table" style="border-top: 1px solid #ccc;">
                            <thead>
                            <tr style="line-height:25px;">
                                ...
                                <th class="uk-width uk-text-left" style="width:300px; ">Observações</th>
                            </tr>
                            </thead>
                        </table>
...
// data da ultima parcela paga
                                    ...
                                    <tr style="line-height:25px;">
                                        ...
                                        <td class="uk-width uk-text-center" style="width:100px;">
                                            <?php echo tool::CompletaZeros(10, $list_seguro->current()->convenios_id . "." . $list_seguro->current()->matricula); ?>
                                        </td>
                                        <td class="uk-width uk-text-left uk-text-uppercase uk-text-overflow" style="width:310px;max-width: 310px;">
                                        ...
                                        <td class="uk-width uk-text-left" style="width:300px;">
                                            <?php echo tool::CompletaZeros(10, $list_seguro->current()->obs); ?>
                                        </td>
                                    </tr>
                                    <?php $seg++;
                                    $list_seguro->next(); endwhile; ?>
                                </tbody>
                            </table>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    jQuery(function() {
        ...
        jQuery("#Btn_excel_seg").click(function(event) {
            ...
            jQuery.ajax({
                async: true,
                url: "util/excel/app_exp/Controller_seguro.php",
                type: "post",
                success: function(resultado) {
 ...
 jQuery("#protestos").on("click","[data-action=remove]",function(){
    
    var $this = $(this);

    var data="action=remove_protesto&id="+$this.data("id");
    console.log(data)
    ...
    jQuery.ajax({
        ...
        success: function(resultado) {
            ...
            if(jQuery.isNumeric(resultado)){
                ...
                $this.closest ("tr").fadeOut (1000).remove ();
                
                UIkit.notify("Protesto removido", {status:'success',timeout: 2500});
