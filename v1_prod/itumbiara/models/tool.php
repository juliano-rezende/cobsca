   ...
   // VALIDA A ALTURA DA TELA
	static function valid_heigth_screen($height)
	{
		...
		if($height<600){
			...
			echo'Resolução minima é de 600px de altura de tela!';
			exit();
...
NATURAZA DO ESTÁGIO
*/
    static function statusCobranca($mod=0,$id="")
    {

        $opt="";
        $arr_nat = array('Em analise' =>'0', 'Aceito' =>'1', 'Recusado' =>'2', 'Protestado' =>'3', 'Quitado' =>'4');
        $arr_bdg = array('<div class="uk-badge uk-badge-notification">Em analise</div>',
                         '<div class="uk-badge uk-badge-primary">Aceito</div>',
                         '<div class="uk-badge uk-badge-danger">Recusado</div>',
                         '<div class="uk-badge uk-badge-warning">Protestado</div>',
                         '<div class="uk-badge uk-badge-success">Quitado</div>');

        /* MODULO DE SELEÇÃO*/
        if($mod == 0){
            foreach ($arr_nat as $key => $value) {
                if($id == $value){$selected='selected="true"';}else{$selected='';}
                $opt.='<option value="'.$value.'" '.$selected.'>'.$key.'</option>';
            }
        }
        /* MODULO DE EXIBIÇÃO*/
        elseif($mod == 1){
