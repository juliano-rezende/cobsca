...
<form method="post" id="Frmdependente" class="uk-form">
...
<fieldset>
    ...
    <label>
		...
		if($FRM_action == "edit"): echo tool::CompletaZeros(3,$dependentes[0]->empresas_id);else:echo"000";endif; ?>" readonly="readonly" >

        <input name="Dep_Convenio_id" type="text" class="input_text center w_40"  id="Dep_Convenio_id"  value="<?php
		if($FRM_action == "edit"): echo tool::CompletaZeros(3,$dependentes[0]->convenios_id);else:echo"000";endif; ?>" readonly="readonly" >
		...
		if($FRM_action == "edit"): echo tool::CompletaZeros(3,$dependentes[0]->sequencia);else:echo"0";endif; ?>" readonly="readonly" >
        <input name="Dep_id" type="hidden"  id="Dep_id"  value="<?php  if($FRM_action == "edit"): echo $dependentes[0]->dep_id; endif; ?>" >
    </label>
<!-- -->
	<label>
    	<span>Nome completo</span>
        <input value="<?php if($FRM_action == "edit"){echo $dependentes[0]->nome;}else{} ?>" type="text" class=" w_400" name="Dep_nome" id="Dep_nome"  />
    </label>
<!-- -->
    <label>
        <span>Data Nasc</span>
        <input  value="<?php if($FRM_action == "edit"):$now = new ActiveRecord\DateTime($dependentes[0]->dt_nascimento);echo $now->format('d/m/Y'); endif; ?>"  type="text" class="uk-text-center w_100 center" name="Dep_data_nasc" id="Dep_data_nasc" placeholder="00/00/0000"/>
        <?php
        if($FRM_action == "edit"):
...
<!-- -->
	<label>
        ...
        <input value="<?php if($FRM_action == "edit"):echo tool::MascaraCampos("???.???.???-??",$dependentes[0]->cpf); endif; ?>" type="text" class="uk-text-center w_100 " name="Dep_cpf" id="Dep_cpf" placeholder="000.000.000-00"/>
    </label>
<!-- -->
	<label>
...
<a  id="Btn_dep_1" class="uk-button uk-button-primary" style="right: 10px; margin-right:5px;line-height: 40px; width: 120px;bottom:30px; position:absolute; z-index: 2;" data-uk-tooltip="{pos:'left'}" title="Salvar Alterações" data-cached-title="Salvar Alterações" ><i class="uk-icon-pencil "></i> Salvar</a>

<a  id="Btn_dep_2" class="uk-button uk-button-primary" style=" right: 10px; margin-right:5px;line-height: 40px; width: 120px;bottom:30px; position:absolute;z-index: 2;" data-uk-tooltip="{pos:'left'}" title="Gravar Novo " data-cached-title="Gravar Novo " ><i class="uk-icon-floppy-o"></i> Gravar</i></a>
...
jQuery(function() {
 ...
 jQuery.ajax({
				...
				type: "post",
				data:'acao=edit&'+$("#Frmdependente").serialize(),
				success: function(resultado) {
		                        ...
		                        /* se for = 0 indica que houve erro ai retornamo o erro na tela do usuario*/
		                            ...
		                            jQuery("#"+jQuery("#Frmdependente,#GridDependentes").closest('.Window').attr('id')+"").remove();

		                            New_window('users','730','500','Dependentes','assets/dependente/Veiw_dependente.php?matricula='+obj.matricula+'',true,false,'Carregando...');
