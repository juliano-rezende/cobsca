// seta o placeholder em todos os selects do forumlario
jQuery(function ($) {
    //function for placeholder select
    function selectPlaceholder(selectID) {
        var selected = $(selectID + ' option:selected');
        var val = selected.val();
        $(selectID + ' option').css('color', '#333');
        selected.css('color', '#999');
        if (val == "") {
            $(selectID).css('color', '#999');
        }
        ;
        $(selectID).change(function () {
            var val = $(selectID + ' option:selected').val();
            if (val == "") {
                $(selectID).css('color', '#999');
            } else {
                $(selectID).css('color', '#333');
            }
            ;
        });
    };
    selectPlaceholder('.select');
});

jQuery(document).ready(function () {

// mascara para os campos
    jQuery("#cpf").mask("999.999.999-99");
    jQuery("#fone_fixo,#fone_trab").mask("(99) 9999-9999");
    jQuery("#fone_cel").mask("(99) 99999-9999");
    jQuery("#cep").mask("99.999-999");
//jQuery("#matricula").css("background-color","#f5f5f5")

    jQuery("#tx_adesao").maskMoney({
        showSymbol: true,
        symbol: "",
        decimal: ",",
        thousands: "."
    });

});// fim document ready

jQuery("#Btn_vendor_0").click(function () {
    New_window('search', '980', '500', 'Pesquisa Vendedor', 'assets/vendedor/Frm_pesquisa_vendedor.php', true, false, 'Carregando...');
});

jQuery("#Btn_vendor_1").click(function () {
    LoadContent('assets/vendedor/Frm_vendedor.php', 'content');
});

// grava os dados no banco de dados

jQuery(function () {

    jQuery("#Btn_vendor_2").click(function (event) {

        // mensagen de carregamento
        jQuery("#msg_loading").html(" Aguarde ... ");

        //abre a tela de preload
        modal.show();

        //desabilita o envento padrao do formulario
        event.preventDefault();

        jQuery.ajax({
            async: true,
            url: "assets/vendedor/Controller_vendedores.php",
            type: "post",
            data: jQuery("#Frmvendedor").serialize(),
            success: function (resultado) {
                console.log(resultado);
                if (jQuery.isNumeric(resultado)) {
                    // mensagen de carregamento
                    jQuery("#msg_loading").html(" Carregando ");
                    // variavel com id
                    var id = resultado;
                    // recarrega a pagina
                    LoadContent('assets/vendedor/Frm_vendedor.php?vendor_id=' + id.replace(/[^\d]+/g, '') + '', 'content');
                } else {
                    //abre a tela de preload
                    modal.hide();
                    UIkit.notify("" + resultado + "", {status: 'danger', timeout: 2500})
                }
            },
            error: function () {
                UIkit.modal.alert("Erro ao enviar dados! Erro 404");/*erro de caminho invalido do arquivo*/
                modal.hide();
            }

        });

    });

});