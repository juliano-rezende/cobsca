...
# Co-Autor......: Marcelo Caja�ba
# Funcao........: Gerar o arquivo de remessas padr�o CNAB-240 da Caixa Economica Federal
...
# ******************************************************************************************************************************************************************
/*Campos Num�ricos (�Picture 9�)
� Alinhamento: sempre � direita, preenchido com zeros � esquerda, sem m�scara de edi��o;
� N�o utilizados: preencher com zeros.
*/
# ******************************************************************************************************************************************************************
...
# ******************************************************************************************************************************************************************
/*
Campos Alfanum�ricos (�Picture X�)
� Alinhamento: sempre � esquerda, preenchido com brancos � direita;
� N�o utilizados: preencher com brancos;
� Caracteres: mai�sculos, sem acentua��o, sem ��, sem caracteres especiais.
*/
# ******************************************************************************************************************************************************************
...
$fusohorario     = 3; // como o servidor de hospedagem � a dreamhost pego o fuso para o horario do brasil
...
$lote_sequencial = 1;                                     // n� do lote. Sempre sera = 1
$lote_servico    = 1;                                     // lote de servico. sera = 1
$header          = '';                                    // 1� linha do arquivo = header
$header_lote     = '';                                    // 2� linha do arquivo = header de lote
...
$dadosempresa=empresa::find_by_cdempresa($SCA_Id_empresa);
$dadosconta=conta::find_by_ativo_and_cdempresa_and_banco(1,$SCA_Id_empresa,104);
$dadosconfig=config::find_by_cdempresa($SCA_Id_empresa);




$lote = remessa::find_by_sql("SELECT MAX((lote)+1) AS novolote FROM  tbremessas WHERE cdbanco = '104' and cdempresa=".$SCA_Id_empresa."");
...
$cpf_cnpj                   = $dadosempresa->current()->cnpj;           // cnpj da empresa
$agencia                    = completazeras(4,$dadosconta->agencia);    // agencia
$dv_agencia                 = '3';                        				// digito verificador da agencia
$codigo_beneficiario        = $dadosconta->codcedente;     				// Codigo do cedente / beneficiario
$empresa_beneficario        = $dadosconta->razaosocial;        			// nome da empresa
$numero_sequencial_arquivo  = 1;                          				// N� remessa tem que ser sequencial e unico
$xid_remessa                = picture_9($numero_sequencial_arquivo,7);  // N� da remessa
$arquivo                    = "CAMINHO ONDE � APRA SALVAR O ARQUIVO/E".$xid_remessa.".REM";    				// nome do arquivo de remessa a ser gerado
...
$header .= '2';                                         // 05.0 -> Tipo de inscri��o do beneficiario : um se pessoa fisico (1) ou juridica (2)
$header .= picture_9($cpf_cnpj,14);                     // 06.0 -> N� de Inscri��o do  Beneficiario cpf ou cnpj
...
$header .= picture_9($codigo_beneficiario,6);           // 10.0 -> Cod. do benefici�rio fornecido pelo banco - N� do cedente
...
$header .= complementoRegistro(10,"brancos");           // 15.0 -> 10 espa�os em banco
...
$header_lote .='01';                                         // 05.1 -> Preencher com �01', se Cobran�a Registrada; ou �02�, se Cobran�a Sem Registro/Servi�os
$header_lote .= complementoRegistro(2,"zeros");              // 06.1 -> Preencher com zeros
$header_lote .='030';                                        // 07.1 -> No. da vers�o do layout. Preencher com 030
$header_lote .= complementoRegistro(1,"brancos");            // 08.1 -> Preencher com espacos
$header_lote .= '2';                                         // 09.1 -> Tipo de inscri��o do beneficiario : um se pessoa fisico (1) ou juridica (2)
$header_lote .= picture_9($cpf_cnpj,15);                     // 10.1 -> CNPJ = N�mero de Inscri��o do  Benefici�rio cpf ou cnpj
$header_lote .= picture_9($codigo_beneficiario,6);           // 11.1 -> COD. CEDENTE ou COD. DO CONVENIO NO BANCO = c�digo do benefici�rio fornecido pelo banco
...
$header_lote .= picture_9($codigo_beneficiario,6);           // 14.1 -> CEDENTE = Cod do benefici�rio fornecido pelo banco
$header_lote .= complementoRegistro(7,"zeros");              // 15.1 -> Cod fornecido pela CAIXA/Gr�fica,utilizado se boleto personalizado; do contr�rio,preencher com zeros
...
$query_titulos=titulos::find_by_sql("SELECT * FROM tbtitulos WHERE (dtemissao = '".date("Y-m-d")."' or dtatualizacao = '".date("Y-m-d")."')  AND stflagrem = '1' ");

$list_titulo= new ArrayIterator($query_titulos);

$num_seg_linha_p_q_r = 1;
$total_boletos = count($query_titulos );

##$total_boletos = 1;  // quantidade de boletos a serem registrados no arquivo de remessa

while($list_titulo->valid()):   // loop para obter os dados dos boletos da sua base de dados e montar as linhas P, Q e R do arquivo de remessa
	...
	$now_venc  = new ActiveRecord\DateTime($list_titulo->current()->dtvencimento);// formata data de vencimento
    $dtvencimento = $now_venc->format('dmy');

    $now1	   = new ActiveRecord\DateTime($list_titulo->current()->dtemissao);		 // formata data de emiss�o d titulo
    $dtemissao=$now1->format('dmy');

    $valor_formatado=limpamoney(number_format($list_titulo->current()->vlrnominal,2,',','.') );
    $valor_formatado=limpamoney($valor_formatado);

	$jur = round(((($list_titulo->current()->vlrnominal /100)*$dadosconfig->txatrazo)/30),2);
    $jur_formatado=limpamoney(number_format($jur,2,',','.') );


	$nosso_numero                    = $list_titulo->current()->nossonumero;         		// nosso numero do seu boleto
	$numero_documento                = $nosso_numero;    							 		// nosso numero do seu boleto
	$data_vencimento_boleto          = $dtvencimento;      							 		// data de vencimento do boleto
	$data_multa                      = '06022017';       							 		// data da multa
	$data_emissao_boleto             = $dtemissao;      								 		// data da emissao do boleto
	$valor_boleto                    = $valor_formatado;				             		// valor nominal do titulo ==> 35000 ==> // 350,00
	$data_juros                      = '06022017';       							 		// data a partir daqual incidira juros
	$i = 2;
	$valor_juros                     = $jur_formatado;      						 		// 0034 ou 350,00, depende se em valor ou em taxa
	$data_desconto                   = '00000000';       							 		// desconto ate o dia.......
	...
	$tipo_inscricao_pagador          = $list_titulo->current()->tpsacado;			 		// tipo de inscri��o do pagador 1 pessoa fisica 2 pessoa juridica
    $numero_inscricao_pagador        = $list_titulo->current()->cpfcnpjsacado;   	 		// cpf
	$nome_pagador                    = remover_acentos($list_titulo->current()->sacado); 	// nome
	$endereco_pagador                = remover_acentos($list_titulos->current()->logradouro).",".$list_titulos->current()->num;   // endereco
	$bairro_pagador                  = remover_acentos($list_titulos->current()->bairro);   // bairro
	$cep_pagador                     = left($list_titulos->current()->cep,5);          		// cep prefixo
	$cep_pagador_sufixo              = right($list_titulos->current()->cep,3);            	// cep sufixo
	$cidade_pagador                  = remover_acentos($list_titulos->current()->cidade);   // cidade
	$estado_pagador                  = remover_acentos($list_titulos->current()->uf);       // estado
	$email_pagador                   = 'cartaofamiliar@hotmail.com';  						// email
	...
	$linha_p .= picture_9($num_seg_linha_p_q_r,5);       // 04.3P -> SERVICO. N� Sequencial do Registro no Lote. (G038). EVOLUIR DE 1 EM 1 PARA CADA SEGMENTO DO LOTE
	$linha_p .= 'P';                                     // 05.3P -> SERVICO. C�d. Segmento do Registro Detalhe, PREENCHER P
	...
	$linha_p .= picture_9($codigo_beneficiario,6);       // 10.3P -> COD. ID. BENEFICIARIO. Cod. do convenio no banco ou C�d. CEDENTE. 
	...
	$linha_p .='1';                                      // 14.3P -> codigo da carteira (1) equivale a cobran�a simples (C006)
	$linha_p .='1';                                      // 15.3P -> Forma de Cadastramento do T�tulo no Banco. 1=cobranca registrada / 2=cobranca sem registro
	$linha_p .='2';                                      // 16.3P -> Tipo de Documento - Preencher '2� (equivale a Escritural)
	$linha_p .='2';                                      // 17.3P -> Identifica��o da Emissao do boleto. 1 = Banco emite/ 2 = Cliente emite (C009)
	...
	$linha_p .= picture_9($data_vencimento_boleto,8);    // 20.3P -> Data de vencimento do t�tulo, no formato DDMMAAAA (Dia, M�se Ano);
	$linha_p .= picture_9($valor_boleto,15);             // 21.3p -> Valor nominal do t�tulo,utilizando 2 casas decimais (exemplo:t�tulo de valor 530,44 - preencher 0000000053044)
	$linha_p .= complementoRegistro(5,"zeros");          // 22.3P -> Ag�ncia Encarregada da Cobran�a (Preencher com zeros)
	$linha_p .= complementoRegistro(1,"zeros");          // 23.3P -> DV (Preencher com zeros)
	$linha_p .= picture_x('99',2);                       // 24.3P -> Esp�cie do T�tulo (NF: NOTA FISCAL, DD:DOCUMENTO DE DIVIDA, CPR: C�DULA DE PRODUTO RURAL, OU:OUTROS = 99
	$linha_p .= picture_x('N',1);                        // 25.3P -> Aceite. preencher com �A� (Aceite) ou�N� (N�o Aceite)
	$linha_p .= picture_9($data_emissao_boleto,8);       // 26.3P -> Data de emissj�o do t�tulo, no formato DDMMAAAA (Dia, M�se Ano);
	$linha_p .= picture_9('2',1);                        // 27.3P -> Juros de mora;preencher com o tipo de prefer�ncia:�1� (Valor por Dia); ou �2� (Taxa Mensal); ou �3�(Isento)
	$linha_p .= picture_9($data_juros,8);                // 28.3P -> Data para in�cio da cobran�a de Juros de Mora, no formato DDMMAAAA (Dia, M�s e Ano). 0 = dia posterior venc. 
	                                                     //          devendo ser maior que a Data de Vencimento; ATEN��O, caso a informa��o seja inv�lida ou n�oinformada, 
										                 //          o sistema assumir� data igual � Datade Vencimento + 1
	...
	$linha_p .= '3';                                     // 36.3P -> C�digo para Protesto. 1 = protestar / 3 = nao protestar
	$linha_p .= '00';                                    // 37.3P -> Prazo para protesto. Numero de dias para  Protesto
	$linha_p .= '1';                                     // 38.3P -> C�digo p/ Baixa/Devolu��o: Preencher - vencido: '1� (Baixar/ Devolver) ou �2� (N�o Baixar / N�o Devolver
	...
	$linha_p .= chr(13).chr(10);                         // essa � a quebra de linha
	...
	$linha_q .= picture_9($lote_servico,4);              // 02.3Q -> Lote de servi�o
	$linha_q .= '3';                                     // 03.3Q -> tipo de registro. Equivalente a detalhe de lote. preencher '3'
	$linha_q .= picture_9($num_seg_linha_p_q_r,5);       // 04.3Q -> N� Sequencial do Registro no Lote
	$linha_q .= 'Q';                                     // 05.3Q -> C�d. Segmento do Registro Detalhe
	$linha_q .= complementoRegistro(1,"brancos");        // 06.3Q -> Espaco
	$linha_q .= picture_9('01',2);                       // 07.3Q -> Cod de Movimento Remessa
	$linha_q .= $tipo_inscricao_pagador;                 // 08.3Q -> Tipo de Inscricao do Pagador (1) CPF (pessoa f�sica) (2) CNPJ Pessoa jur�dica
	...
	$linha_q .= '0';                                     // 17.3Q -> Tipo de Inscri��o do sacador AVALISTA (0) nenhum (1) CPF (pessoa f�sica) (2) CNPJ Pessoa jur�dica
	...
	$linha_q .= chr(13).chr(10);                         // essa � a quebra de linha
	...
	$linha_r .= picture_9($lote_servico,4);              // 02.3R -> Lote de servi�o
	$linha_r .= '3';                                     // 03.3R -> tipo de registro. Equivalente a detalhe de lote. preencher '3'
	$linha_r .= picture_9($num_seg_linha_p_q_r,5);       // 04.3R -> N� Sequencial do Registro no Lote
	$linha_r .= 'R';                                     // 05.3R -> C�d. Segmento do Registro Detalhe
	...
	$linha_r .= chr(13).chr(10);                         // essa � a quebra de linha
...
$linha_5 .= chr(13).chr(10);                         // essa � a quebra de linha
...
$linha_9 .= '9999';                                  // 02.9 -> lote de servi�o. Preencher '9999'
...
if (!$handle = fopen($filename, 'w+')){
	erro("<br>&nbsp;N�o foi poss�vel abrir o arquivo ($filename)");
}

if (fwrite($handle, "$conteudo") === FALSE){
	echo "<br>&nbsp;N�o foi poss�vel escrever no arquivo ($filename)";
}
