...
<table style="width: 100%;" class="uk-table">
  ...
  <thead >
    <tr>
        <td align="center" colspan="6" style=" font-weight: bold;; font-size: 18px; text-transform: uppercase;" >'.$dadosboleto["identificacao"].'</td>
    </tr>
     <tr>
        <td align="center" colspan="6" >CNPJ: '.isset($dadosboleto["cpf_cnpj"]) ? $dadosboleto["cpf_cnpj"] : '' ?></td>
    </tr>
    <tr>
        <td align="center" colspan="6">ENDEREÇO: '.$dadosboleto["endereco"].' '.$dadosboleto["cidade_uf"].'</td>
    </tr>
    <tr>
...
?>
    <tr style="line-height: 30px;">
        ...
        <td align="center" style="border-bottom:1px dashed #e5e5e5">'.$listfat->current()->matricula.'</td>
        <td align="left"  style="border-bottom:1px dashed #e5e5e5">'.strtoupper($listfat->current()->nm_associado).'</td>
        <td align="center"  style="border-bottom:1px dashed #e5e5e5">'.tool::Referencia($ref->format('Ymd'),"/").'</td>
        <td align="center"  style="border-bottom:1px dashed #e5e5e5">

           '.number_format($listfat->current()->valor,2,",",".").'

        </td>
        <td align="center"  style="border-bottom:1px dashed #e5e5e5">

           '.number_format($listfat->current()->valor_pro,2,",",".").'

        </td>
        <td align="center"  style="border-bottom:1px dashed #e5e5e5">'.number_format($listfat->current()->valor_pro+$listfat->current()->valor,2,",",".").'</td>
...
?>
  ...
  <tfoot>
    <tr style="line-height: 30px;">
        ...
        <th align="center"  style="border-bottom:1px dashed #e5e5e5" >'.number_format($vl_t+$valor_t_pro,2,',','.').'</th>
    </tr>
  </tfoot>
 </table>
 </div>
