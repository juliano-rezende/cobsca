...
$Query_Associado = associados::find_by_sql("SELECT
	...
	convenios.tx_adesao as tx_adesao_padrao,
    vendedores.tx_adesao as tx_adesao_vendedor
	FROM
	associados
	INNER JOIN convenios ON convenios.id = associados.convenios_id
    INNER JOIN vendedores ON vendedores.id = associados.vendedores_id
	WHERE
...
// não existe então vamos criar
    ...
    if ($FRM_tx_adesao > 0) {
        ...
        $adesao = ($Query_Associado[0]->tx_adesao_padrao - $Query_Associado[0]->tx_adesao_vendedor);
            ...
            $create_adesao = faturamentos::create(
                array('matricula' => $FRM_matricula,                // matricula do associado
                    ...
                    'valor' => $adesao,    //valor da taxa de adesao
                    'usuarios_id' => $COB_Usuario_Id,
                    'empresas_id' => $COB_Empresa_Id,
