...
$Query_dados = dados_cobranca::find_by_sql("	SELECT
											...
											WHERE
												dados_cobranca.matricula = '" . $FRM_matricula . "' AND dados_cobranca.status='1'");

?>
<style>
    #Frmdadoscobranca label span{width: 100px;}
</style>
...
<form method="post" id="Frmdadoscobranca" class="uk-form">
    ...
    <fieldset style="width:450px;">
        ...
        <label>
            ...
            <input type="hidden" value="<?php if ($Query_dados) {
                echo $Query_dados[0]->id;
            } ?>" id="dados_cobranca_id" name="dados_cobranca_id"/>
            <input type="hidden" value="<?php echo $FRM_matricula ?>" id="dados_cobranca_matricula" name="dados_cobranca_matricula"/>
            <input type="hidden" value="<?php if ($Query_dados) {
                echo $Query_dados[0]->forma_cobranca_id;
            } ?>" id="confirm_forma_cobranca" name="confirm_forma_cobranca"/>
            <input type="hidden" value="<?php if ($Query_dados) {
                echo $Query_dados[0]->forma_cobranca_sys_id;
            } ?>" id="forma_cobranca_sys_id" name="forma_cobranca_sys_id"/>
            <select class="select w_300" name="formadecobranca" id="formadecobranca">
                ...
                if ($Query_dados) {
                    ...
                    while ($formas->valid()):
                        ...
                        if ($Query_dados[0]->forma_cobranca_id == $formas->current()->id) {
                            $select = 'selected="selected"';
                        } else {
                            $select = "";
                        }

                        echo '<option  ' . $select . ' value="' . $formas->current()->id . '-' . $formas->current()->forma_cobranca_sys_id . '">' . strtoupper(($formas->current()->descricao)).'</option>';
                    ...
                    $formas = new ArrayIterator($Query_formas);
                    while ($formas->valid()):
                        echo '<option  value="' . $formas->current()->id . '-' . $formas->current()->forma_cobranca_sys_id . '" >' . strtoupper(($formas->current()->descricao)).'</option>';
                        $formas->next();
                    endwhile;
            ...
            <select class="select w_300" name="planos_id" id="planos_id">
                ...
                if ($Query_dados) {
                    ...
                    while ($planos->valid()):
                        ...
                        if ($Query_dados[0]->plano_id == $planos->current()->id) {
                            $select = 'selected="selected"';
                        } else {
                            $select = "";
                        }

                        echo '<option  value="' . $planos->current()->id . '" ' . $select . ' >' . strtoupper(utf8_encode($planos->current()->descricao)) . ' - ' . number_format($planos->current()->valor, 2, ",", ".").'</option>';
                        $planos->next();
                    endwhile;
                }
                ?>
        ...
        <label id="l_card" style="<?php  if($Query_dados[0]->forma_cobranca_sys_id == 6 || $Query_dados[0]->forma_cobranca_sys_id == 8){echo 'display:block;';}else{echo 'display:none;';} ?>">
            ...
            <select class="select w_300" name="associados_card_id" id="associados_card_id">
                ...
                if ($Query_dados) {
                    ...
                    $Cards = associados::find_by_sql("SELECT * FROM associados_cards WHERE matricula= '".$FRM_matricula."'");

                    $count = count($Cards);

                    if($count > 0) {
                        echo'<option>Selecione um cartão</option>';
                        ...
                        while ($CardsList->valid()):
                            ...
                            echo '<option  value="' . $CardsList->current()->id . '" ' . $select . ' >' . $card_number . '</option>';
                            $CardsList->next();
                        endwhile;
                    }else{
                        echo'<option>Não há cartões cadastrados.</option>';
                    }
...
<a id="Btn_confirm_cob" class="uk-button" style="position:absolute;; right:15px; bottom:30px; z-index:2;" data-uk-tooltip="{pos:'left'}" title="Confirmar dados" data-cached-title="Confirmar dados"><i class="uk-icon-check"></i> Confirmar Dados</a>
...
<script type="text/javascript">
    ...
    jQuery("#formadecobranca").change(function () {
        ...
        var new_height = window_on + 110;/* soma mais 110px ao tamanho atual da div*/
        ...
        jQuery.post("assets/planos/ajax_planos.php", {acao: 0, forma_cobranca_id: res[0]}, function (valor) {
            jQuery("#planos_id").html(valor);
        });
    ...
    jQuery("#Btn_add_card").click(function(){
        New_window('file-text-o','500','250','Adicionar Cartão','assets/faturamento/Frm_add_card.php?mat=<?=$FRM_matricula; ?>&dc=<?=$Query_dados[0]->id;?>&a_c_id=<?=$Query_dados[0]->api_cob_cliente_id;?>',true,false,'Carregando...');
    });
</script>