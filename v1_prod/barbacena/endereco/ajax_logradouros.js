/* funções para pesquisa de adição de novos endereços */
	...
	jQuery("select[name=uf]").change(function(){
		...
		if(jQuery(this).val() == "" || jQuery(this).val() == 0){
			...
			jQuery("select[name=cidade]").html('<option value="">Selecione uma cidade.</option>');
			jQuery("select[name=bairro]").html('<option value="">Selecione um bairro</option>');
			jQuery("select[name=logradouro]").html('<option value="">Selecione um logradouro</option>');
			return false;
	...
	jQuery("select[name=cidade]").change(function(){
		...
		if(jQuery(this).val() == "" || jQuery(this).val() == 0){
			...
			jQuery("select[name=bairro]").html('<option value="">Selecione uma cidade</option>');
			jQuery("select[name=logradouro]").html('<option value="">Selecione um logradouro</option>');
			return false;
	...
	jQuery("select[name=bairro]").change(function(){
		...
		if(jQuery(this).val() == "" || jQuery(this).val() == 0){
			jQuery("select[name=logradouro]").html('<option value="">Selecione um bairro</option>');
			return false;
		}// retorna falso se for vazio
